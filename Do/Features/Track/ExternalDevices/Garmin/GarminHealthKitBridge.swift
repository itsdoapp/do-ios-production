//
//  GarminHealthKitBridge.swift
//  Do
//
//  Bridge Garmin data via HealthKit
...
class GarminHealthKitBridge {
    static let shared = GarminHealthKitBridge()
    
    private let healthStore = HKHealthStore()
    
    private init() {}
    
    func syncGarminDataToHealthKit(metrics: WorkoutMetrics) {
    ...
    private func writeHeartRate(_ bpm: Double) {
        ...
        healthStore.save(sample) { success, error in
            if let error = error {
                print("❌ [GarminHealthKitBridge] Error saving heart rate: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func writeDistance(_ meters: Double) {
        ...
        healthStore.save(sample) { success, error in
            if let error = error {
                print("❌ [GarminHealthKitBridge] Error saving distance: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func writeCalories(_ kcal: Double) {
        ...
        healthStore.save(sample) { success, error in
            if let error = error {
                print("❌ [GarminHealthKitBridge] Error saving calories: \(error.localizedDescription)")
            }
        }
    }
}
