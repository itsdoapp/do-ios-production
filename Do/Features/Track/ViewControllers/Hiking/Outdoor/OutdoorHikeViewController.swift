...
// SwiftUI view for hiking tracking interface
struct OutdoorHikeTrackerView: View {
    @ObservedObject var viewModel: OutdoorHikeViewController
    ...
    var body: some View {
        GeometryReader { geometry in
                ...
                headerView
                    ...
                    .background(getBackgroundForHikeType(viewModel.hikeType))
                        ...
                        .padding(.horizontal, 20)
                            ...
                            .shadow(color: Color.black.opacity(0.15), radius: 6, x: 0, y: 3)
                                ...
                                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                    ...
                                    statsCard(
                                        title: "PACE",
                                        value: engine.formattedPace,
                                        unit: engine.getPaceUnitString(),
                                        icon: "speedometer",
                                        iconColor: .purple
                                    )
                                    ...
                                    statsCard(
                                        title: "ELEVATION",
                                        value: "\(Int(engine.elevationGain.value))",
                                        unit: engine.getElevationUnitString(),
        ...
        .background(
            getBackgroundForHikeType(viewModel.hikeType)
        )
    }
    ...
    var headerView: some View {
        // Remove the GeometryReader wrapper - this is likely causing layout issues
            ...
            Text(viewModel.hikeType.rawValue)
                .font(.title2)
                .fontWeight(.bold)
                    .foregroundColor(.white)
        ...
        .background(
            LinearGradient(
                gradient: Gradient(colors: [
                    getBackgroundGradientStart(viewModel.hikeType),
                    Color(hex: "#0A0F1E")
                ]),
                startPoint: .top,
                endPoint: .bottom
    ...
    struct HikeMapCard: View {
        ...
        VStack(spacing: 0) {
            ...
            Button(action: {
                ...
                // Map View
                HikeMapViewContainer(mapView: mapView, onHomeButtonTapped: onHomeButtonTapped, viewModel: viewModel)
    ...
    struct HikeNavigationPanel: View {
    ...
    // MARK: - Hike Map View Container
    struct HikeMapViewContainer: UIViewRepresentable {
        let mapView: MKMapView
        let onHomeButtonTapped: () -> Void
        let viewModel: OutdoorHikeViewController
    ...
    struct MapViewContainer: UIViewRepresentable {
        let mapView: MKMapView
        let onHomeButtonTapped: () -> Void
        let viewModel: OutdoorHikeViewController // Add viewModel
        ...
        func updateUIView(_ uiView: UIView, context: Context) {
            ...
            if viewModel.engine.hikeState == .inProgress && viewModel.pathMarkers.isEmpty {
    ...
    public func getBackgroundForHikeType(_ hikeType: HikeType) -> some View {
        switch hikeType {
        case .trail, .mountain, .nature:
    ...
    public func getBackgroundGradientStart(_ hikeType: HikeType) -> Color {
        switch hikeType {
        case .trail, .mountain, .nature:
    ...
    public func calculatePacePosition(in width: CGFloat) -> CGFloat {
        ...
        let currentPace = engine.pace.value > 0 ? min(max(engine.pace.value, 120), 1800) : targetPace
    ...
    public func calculateHeartRatePosition(in width: CGFloat) -> CGFloat {
        let currentHR = engine.heartRate
...
class OutdoorHikeViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, ObservableObject, WorkoutCommunicationHandlerDelegate {
    
    // MARK: - Properties
    public var engine = HikeTrackingEngine.shared
    ...
    public var hostingController: UIHostingController<OutdoorHikeTrackerView>?
    ...
    @Published var hikeGoal: HikeGoal?
    @Published var currentHeartRateZone: Int = 0
    
    // Run configuration
    var hikeType: HikeType = .nature
    ...
    weak var delegate: OutdoorHikeViewControllerDelegate?
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 13.0, *) {
            ...
            statusBarView.backgroundColor = UIColor(getBackgroundGradientStart(hikeType))
            view.addSubview(statusBarView)
        }
        
        // Get the shared run engine
        engine = HikeTrackingEngine.shared
        ...
        print("OutdoorHikeViewController loaded successfully")
        
        // Check for saved incremental state first
        let restoredState = engine.restoreIncrementalState()
        ...
        if restoredState {
            ...
            isRunning = engine.hikeState == .inProgress
            isPaused = engine.hikeState == .paused
            updateStatusDisplay()
            ...
            if isJoiningExistingWorkout {
                ...
                if engine.ensureTimerIsRunning() {
                ...
                } else if engine.hikeState == .paused {
    ...
    @objc public func handleAppWillTerminate() {
        ...
        if engine.hikeState == .inProgress || engine.hikeState == .paused {
            engine.saveIncremental()
        }
    }
    ...
    public func setupOutdoorRunNotifications() {
        ...
        print("📱 OutdoorHikeViewController: Set up outdoor hike state change notifications")
    }
    
    @objc public func handleOutdoorRunStateChangeFromWatch(_ notification: Notification) {
        print("📱 OutdoorHikeViewController: Received outdoor hike state change from watch")
        
        guard let userInfo = notification.userInfo,
            let command = userInfo["command"] as? String else {
            print("📱 OutdoorHikeViewController: Missing command in notification")
            return
        }
        
        print("📱 OutdoorHikeViewController: Processing command: \(command)")
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            switch command {
            case "paused":
                if self.engine.hikeState == .inProgress {
                    print("📱 OutdoorHikeViewController: Pausing from watch command")
                    // Call engine directly - this will fire notifications to update UI
                    self.engine.pauseRun()
            ...
            case "inProgress":
                if self.engine.hikeState == .paused {
                    print("📱 OutdoorHikeViewController: Resuming from watch command")
                    // Call engine directly - this will fire notifications to update UI
                    self.engine.resumeRun()
            ...
            case "completed":
                if self.engine.hikeState == .inProgress || self.engine.hikeState == .paused {
                    print("📱 OutdoorHikeViewController: Ending from watch command")                    
                    // Update UI state immediately
                    self.isRunning = false
                    self.isPaused = false
            ...
            default:
                print("📱 OutdoorHikeViewController: Unknown command: \(command)")
            }
        }
    }
    ...
    @objc public func handleWatchConnectivityChanged(_ notification: Notification) {
        ...
        DispatchQueue.main.async {
            if isReachable {
                ...
                self.engine.evaluatePrimarySource()
            }
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if engine.hikeState == .inProgress || engine.hikeState == .paused {
            print("⏱️ Syncing UI timer on viewWillAppear - engine time: \(String(format: "%.2f", engine.elapsedTime)), display time: \(String(format: "%.2f", displayElapsedTime))")
            ...
            if engine.hikeState == .inProgress && uiTimer == nil {
                startUITimer()
            }
        }
    }
    ...
    public func updateUIWithLatestMetrics() {
        ...
        guard !isPaused || engine.hikeState != .paused else { return }
    ...
    @objc public func handleRunMetricsUpdate() {
        ...
        let engineElapsedTime = engine.elapsedTime
    ...
    public func setupHostingController() {
        ...
        let outdoorRunTrackerView = OutdoorHikeTrackerView(
            viewModel: self,
    ...
    public func setupMetricsCoordinator() {
        metricsCoordinator = MetricsCoordinator(hikeEngine: engine)
    }
    ...
    public func setupMapStyleForHikeType() {
        // Set different map styles and backgrounds based on hike type
        switch hikeType {
        case .trail, .mountain, .nature:
            ...
            // Set route color for outdoor hikes
    ...
    // MARK: - State Handling
    public func handleRunStateChange(_ state: HikeState) {
        switch state {
        case .notStarted:
            isRunning = false
            isPaused = false
    ...
    public func updateReturnRoute(from currentLocation: CLLocationCoordinate2D) {
        guard let startLocation = engine.locationList.first?.coordinate else { return }
        ...
        if engine.locationList.count > 10 {
            ...
            let stepSize = max(1, engine.locationList.count / 4)
            ...
            for i in stride(from: 0, to: engine.locationList.count, by: stepSize) {
                let location = engine.locationList[i]
            ...
            if let firstLocation = engine.locationList.first {
    ...
    public func updateMileMarkers(for distance: Measurement<UnitLength>) {
        ...
        for i in 1...totalMarkers {
            ...
            if let markerLocation = findLocationAtDistance(targetDistance) {
                ...
                annotation.title = engine.useMetric ? "\(i) km" : "\(i) mi"
    ...
    public func findLocationAtDistance(_ targetDistance: Double) -> CLLocation? {
        let locationDataList = engine.locationList
        guard locationDataList.count > 1 else { return nil }
        
        var currentDistance = 0.0
        
        for i in 1..<locationDataList.count {
            let prevLocationData = locationDataList[i-1]
            let currentLocationData = locationDataList[i]
            
            // Convert LocationData to CLLocation for distance calculation
            let prevLocation = CLLocation(
                coordinate: prevLocationData.coordinate,
                altitude: prevLocationData.altitude,
                horizontalAccuracy: prevLocationData.horizontalAccuracy,
                verticalAccuracy: prevLocationData.verticalAccuracy,
                course: prevLocationData.course,
                speed: prevLocationData.speed,
                timestamp: prevLocationData.timestamp
            )
            
            let currentLocation = CLLocation(
                coordinate: currentLocationData.coordinate,
                altitude: currentLocationData.altitude,
                horizontalAccuracy: currentLocationData.horizontalAccuracy,
                verticalAccuracy: currentLocationData.verticalAccuracy,
                course: currentLocationData.course,
                speed: currentLocationData.speed,
            ...
            if newDistance >= targetDistance {
                ...
                let latitude = prevLocationData.coordinate.latitude + fraction * (currentLocationData.coordinate.latitude - prevLocationData.coordinate.latitude)
                let longitude = prevLocationData.coordinate.longitude + fraction * (currentLocationData.coordinate.longitude - prevLocationData.coordinate.longitude)
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if engine.hikeState == .inProgress {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                self.engine.processLocationUpdate(location)
                ...
                DispatchQueue.main.async {
                    ...
                    if !self.routePlanner.routePolyline.isEmpty && self.engine.hikeState == .inProgress {
    ...
    let currentDistanceValue = engine.distance.converted(to: .miles).value
    ...
    public func shouldAnnounceMilestone(distance: Double) -> Bool {
        ...
        guard engine.elapsedTime >= 60 else { return false }
    ...
    public func createProgressAnnouncement() -> String {
        ...
        let speechTime = formatTimeForSpeech(engine.formattedTime)
   ...
   public func createMilestoneAnnouncement(distance: Double, time: String) -> String {
    ...
    public func createTimeBasedAnnouncement(time: String) -> String {
        let distance = engine.formattedDistance
        ...
        case .moderate:
            ...
            if includePaceGuidance {
                announcement += ". Pace: \(engine.formattedPace)"
            }
            return announcement
        ...
        case .comprehensive:
            ...
            if includeHeartRateGuidance, engine.heartRate > 0 {
                announcement += ". Heart rate: \(Int(engine.heartRate))"
            }
            return announcement
    ...
    public func createEncouragementMessage() -> String? {
        let timeInMinutes = engine.elapsedTime / 60
        let currentDistanceValue = engine.distance.converted(to: .miles).value
        let currentPace = engine.pace.value
        ...
        if isAtDistanceMilestone(currentDistanceValue) {
            ...
            let timeString = formatTimeForSpeech(engine.formattedTime)
        ...
        if timeInMinutes > 10 && isPaceConsistent() {
            return "Excellent pace consistency! You're maintaining a steady \(engine.formattedPace) rhythm!"
        }
        
        // Heart rate zone encouragement
        if engine.heartRate > 0 {
            let heartRateZone = getHeartRateZone(engine.heartRate)
            if heartRateZone == "aerobic" && timeInMinutes > 15 {
                return "Perfect aerobic zone at \(Int(engine.heartRate)) BPM! This builds your endurance base!"
            }
        }
    ...
    public func getAverageRecentDistance() -> Double? {
        // Use cached GlobalVariables data
        let outdoorLogs: [HikeLog] = []
    ...
    public func announceHikeSummary() {
        let speechTime = formatTimeForSpeech(engine.formattedTime)
        let elevationGain = engine.elevationGain.converted(to: engine.useMetric ? .meters : .feet).value
        let elevationGainFormatted = String(format: "%.0f %@", elevationGain, engine.getElevationUnitString())
        
        let summary = "Hike completed. Total distance: \(engine.formattedDistance). Time: \(speechTime). Elevation gain: \(elevationGainFormatted). Excellent hike!"
    ...
    func startRun() {
        ...
        engine.hikeType = hikeType
        engine.startRun(hikeType: hikeType)
    ...
    func showRunSummary() {
        // Create and present hike summary
        if let hikeLog = engine.generateHikeLog() {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                let summaryVC = HikeAnalysisViewController()
                summaryVC.hike = hikeLog
                summaryVC.modalPresentationStyle = .fullScreen
                self.present(summaryVC, animated: true)
            }
        } else {
            // If no valid hike data, just dismiss
            dismiss(animated: true)
        }
    }
    
    // MARK: - Load Hike Goals and Historical Data
    public func loadRunGoalAndHistoricalData() {
        print("📱 🚀 OutdoorHikeViewController: LOADING HIKE GOAL AND HISTORICAL DATA")
        
        // Check initial state of global variables
        print("📱 📊 Initial GlobalVariables state:")
        let emptyLogs: [HikeLog] = []
        print("📱 📊 - Hiking logs: \(emptyLogs.count)")
        
        // Load target pace from previous hikes or user settings
        if let savedTargetPace = UserDefaults.standard.object(forKey: "targetPace_\(hikeType.rawValue)") as? Double {
            targetPace = savedTargetPace
            targetPaceSource = "Recent Hikes"
            print("📱 ✅ Loaded saved target pace: \(savedTargetPace)")
        } else {
            print("📱 ❌ No saved target pace found")
        }
        
        // Load hike goal if one was set
        if let goalDistance = UserDefaults.standard.object(forKey: "goalDistance_\(hikeType.rawValue)") as? Double,
           let goalTime = UserDefaults.standard.object(forKey: "goalTime_\(hikeType.rawValue)") as? TimeInterval {
            hikeGoal = HikeGoal(distance: goalDistance, time: goalTime)
            print("📱 ✅ Loaded saved hike goal: \(goalDistance) distance, \(goalTime) time")
        } else {
            print("📱 ❌ No saved hike goal found")
        }
        ...
        // Fetch hiking logs to populate []
        group.enter()
        print("�� 🔄 Fetching hiking logs...")
        getHikingLogs { [weak self] logs, error in
            defer { group.leave() }
            
            if let error = error {
                print("📱 ❌ Error fetching hiking logs: \(error.localizedDescription)")
                return
            }
            
            if let logs = logs {
                print("📱 ✅ Successfully fetched \(logs.count) hiking logs")
                let emptyLogs: [HikeLog] = []
                print("�� 💾 emptyLogs now has \(emptyLogs.count) hikes")
                
                // Log first few hikes for debugging
                for (index, hike) in logs.prefix(3).enumerated() {
                    print("📱 🏔️ Hiking log \(index + 1): pace=\(hike.avgPace ?? "nil"), distance=\(hike.distance ?? "nil"), duration=\(hike.duration ?? "nil"), created=\(hike.createdAt?.description ?? "nil")")
                }
            } else {
                print("📱 ❌ No hiking logs data returned")
            }
        }
        ...
        print("📱 📊 - Hiking logs: \(emptyLogs.count)")
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                // Now load historical paces for this specific hike type
                Task {
                    do {
                        // Get previous hikes of this type using the populated global variables
                        let previousHikes = try await self.getHikeLogsForType(hikeType: self.hikeType.rawValue, limit: 5)
                        
                        print("�� 📊 Got \(previousHikes.count) hikes for type: \(self.hikeType.rawValue)")
                    
                    // Calculate average paces
                        let paces = previousHikes.compactMap { hike -> Double? in
                        // Convert string values to numeric types
                            guard let distanceString = hike.distance,
                              let distanceValue = Double(distanceString.replacingOccurrences(of: " km", with: "").replacingOccurrences(of: " mi", with: "")),
                                  let durationString = hike.duration,
                              let durationValue = self.timeStringToSeconds(durationString),
                              distanceValue > 0, durationValue > 0 else { 
                                print("📱 ❌ Invalid pace data: distance=\(hike.distance ?? "nil"), duration=\(hike.duration ?? "nil")")
                            return nil 
                        }
                    ...
                    await MainActor.run {
                        ...
                        if !paces.isEmpty {
                            ...
                            self.targetPace = averagePace
                                self.targetPaceSource = "Historical Data (\(paces.count) hikes)"
                            let unit = self.engine.useMetric ? "sec/km" : "sec/mi"
                            ...
                            UserDefaults.standard.set(averagePace, forKey: "targetPace_\(self.hikeType.rawValue)")
    ...
    public func getHikeLogsForType(hikeType: String, limit: Int? = nil) async throws -> [HikeLog] {
    ...
    public func updatePaceStatus(currentPace: Double) {
        // engine.pace is in minutes/km, convert to user's preferred units
    ...
    public func checkForPaceGuidance() {
        ...
        guard engine.elapsedTime >= minimumTimeBeforeGuidance else {
            return
        }
        ...
        if currentPaceStatus != .onTarget {
            ...
            engine.coachFeedback = guidanceMessage
    ...
    public func generateIntelligentWaypoints(from coordinates: [CLLocationCoordinate2D]) -> [RouteWaypoint] {
        ...
        for i in 1..<coordinates.count-1 {
            ...
            let checkpointInterval = engine.useMetric ? 1000.0 : 804.67 // 1km or 0.5mi
            if Int(cumulativeDistance / checkpointInterval) > Int((cumulativeDistance - segment) / checkpointInterval) {
                let distanceText = engine.useMetric ?
    ...
    public func updateRouteProgressStats(progressIndex: Int) {
        ...
        if engine.elapsedTime > 0 && completedDistance > 0 {
            let currentPace = engine.elapsedTime / completedDistance // seconds per meter
    ...
    @objc public func startButtonTapped() {
        ...
        if let selectedRoute = preSelectedRoute {
            ...
            engine.startGuidanceForRoute(selectedRoute)
        }
    ...
    public func sendRouteToWatch() {
        ...
        if !routePlanner.routePolyline.isEmpty {
            ...
            // Include type
            routeData["hikeType"] = hikeType.rawValue
        }
        
        // Include current run state if running
        if engine.hikeState == .inProgress || engine.hikeState == .paused {
            routeData["hikeState"] = engine.hikeState == .inProgress ? "running" : "paused"
            routeData["elapsedTime"] = engine.elapsedTime
            routeData["distance"] = engine.distance.value
            routeData["pace"] = engine.pace.value
        }
    ...
    public func updateProgressOverlay() {
        guard !routePlanner.routePolyline.isEmpty,
              engine.hikeState == .inProgress,
              !engine.locationList.isEmpty else {
            return
        }
        
        // Get the current run path from the engine
        let runPath = engine.locationList.map { $0.coordinate }
        ...
        if !routePlanner.routePolyline.isEmpty {
            // Find closest point on route to current location
            if let currentLocation = engine.locationList.last,
               let closestPoint = findClosestPointOnRoute(to: currentLocation.coordinate) {
    ...
    public func showEndRunConfirmation() {
        ...
        guard engine.hikeState == .inProgress || engine.hikeState == .paused else { return }
    ...
    public func updateMilestoneMarkers(for location: CLLocation) {
        let distance = engine.distance.value
        let markerDistanceMeters = engine.useMetric ? 1000.0 : 1609.34 // 1km or 1mi
        ...
        if let startLocation = engine.locationList.first {
            let startAnnotation = MKPointAnnotation()
    ...
    public func createOptimalReturnRoute(from currentLocation: CLLocationCoordinate2D) {
        ...
        if directDistance < 500 || engine.locationList.count < 10 {
    ...
    public func createReversedPathReturnRoute(from currentLocation: CLLocationCoordinate2D, to homeLocation: CLLocationCoordinate2D) {
        ...
        for (index, locationData) in engine.locationList.enumerated() {
        ...
        let simplificationFactor = max(3, engine.locationList.count / 25) // Ensure we don't have too many points
        
        for i in stride(from: closestIndex, through: 0, by: -simplificationFactor) {
            let locationData = engine.locationList[i]
    ...
    func showVoiceGuidanceSettings() {
        let settingsVC = UIHostingController(rootView: VoiceGuidanceHikeSettingsView(viewModel: self))
    ...
    public func setupVoiceAnnouncementTimer() {
        ...
        Timer.scheduledTimer(withTimeInterval: 10.0, repeats: true) { [weak self] _ in
            guard let self = self, 
                  ...
                  self.engine.hikeState == .inProgress,
                  !self.isPaused else { return }
            
            self.checkForVoiceAnnouncement(at: self.engine.elapsedTime)
        }
    ...
    public func setupTrackerView() {
        ...
        let trackerView = OutdoorHikeTrackerView(
            viewModel: self,
            mapView: self.mapView
        )
    ...
    public func populateRecentRunHistory() {
        // Legacy run-history-based analysis removed for hiking;
        // use AWS-backed hike analysis instead (see loadHikeHistory / analyzeHikeHistory).
    }
           ...
           if isRunning && !hasCompletedRun {
               ...
               alert.addAction(UIAlertAction(title: "End Workout", style: .destructive) { [weak self] _ in
                   ...
                   self?.dismiss(animated: true) { [weak self] in
                       self?.delegate?.outdoorHikeWasCanceled()
                   }
               })
    ...
    public func updateCadenceDisplay() {
        ...
        // The main update happens through SwiftUI binding to the HikeTrackingEngine
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // Log for debugging
            if self.engine.cadence > 0 {
                print("📱 Updated cadence display: \(self.engine.cadence) spm")
            }
        }
    }
    ...
    public func updateHeartRateDisplay() {
        DispatchQueue.main.async { [weak self] in
            ...
            if self.engine.heartRate > 0 {
                print("❤️ Heart rate updated: \(Int(self.engine.heartRate)) bpm")
            }
    ...
    public func updateHeartRateZone() {
        ...
        let currentHeartRate = engine.heartRate
    ...
    public func syncTimeWithEngine(forceSync: Bool = false, fromExternal: Bool = false) {
        ...
        let engineTime = self.engine.elapsedTime
    ...
    public func sendTrackingStatusToWatch() {
        ...
        if !WCSession.default.isReachable {
            ...
            engine.handleWatchCommunicationError(
        ...
        let hasValidMetrics = engine.elapsedTime > 0 || engine.distance.value > 0 || engine.hikeState == .inProgress
        
        if !hasValidMetrics && engine.hikeState == .notStarted {
            print("📱 Skipping tracking status - metrics not yet initialized (elapsedTime: \(engine.elapsedTime), distance: \(engine.distance.value), state: \(engine.hikeState.rawValue))")
            return
        }
        ...
        let status: [String: Any] = [
            "type": "trackingStatus",
            "hikeState": engine.hikeState.rawValue,
            "state": engine.hikeState == .inProgress ? "inProgress" : engine.hikeState.rawValue,
            "metrics": metrics,
            "workoutActive": engine.hikeState == .inProgress || engine.hikeState == .paused,
            "isIndoor": engine.isIndoorMode,
            "watchTracking": engine.isWatchTracking,
            "useImperialUnits": !UserPreferences.shared.useMetricSystem
        ]
        
        print("📱 Sending status to watch: state=\(engine.hikeState.rawValue), time=\(engine.elapsedTime)s, distance=\(engine.distance.value)m")
            ...
            timeoutTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: false) { [weak self] _ in
                ...
                if operationCompleted.testAndSet() {
                    ...
                    DispatchQueue.main.async {
                        ...
                        // TODO: Consider notifying HikeTrackingEngine about the timeout
                        // by calling: self?.engine.handleWatchCommunicationError()
                    }
                }
            }
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            if session.activationState == .activated && session.isPaired {
                ...
                let pauseCommand: [String: Any] = [
                    "type": "outdoorHikeStateChange",
                    "command": "paused",
                    "isIndoor": false,
                    "timestamp": Date().timeIntervalSince1970,
                    "workoutId": self.engine.workoutId.uuidString
                ]
    ...
    /// Setup screen lock state bindings with HikeTrackingEngine
    public func setupScreenLockBindings() {
        ...
        // One-way sync: ViewController -> HikeTrackingEngine
        $isScreenLocked
            .removeDuplicates() // Prevent unnecessary updates
            .sink { [weak self] locked in
                guard let self = self else { return }
                // Only update if different to prevent cycles
                if self.engine.isScreenLocked != locked {
                    self.engine.isScreenLocked = locked
                }
            }
            .store(in: &cancellables)
        
        // One-way sync: HikeTrackingEngine -> ViewController
        engine.$isScreenLocked
            .removeDuplicates() // Prevent unnecessary updates
    ...
    // Add this deinit method to the OutdoorHikeViewController class
    deinit {
        print("🗑️ OutdoorHikeViewController deinit - cleaning up resources")
        ...
        print("🗑️ OutdoorHikeViewController cleanup completed")
    }
}

// MARK: - MKMapViewDelegate
// MARK: - WorkoutCommunicationHandlerDelegate
extension OutdoorHikeViewController {
    func workoutCommunicationHandler(_ handler: WorkoutCommunicationHandler, didReceiveMessage message: [String : Any]) {
        // For now, just log and rely on existing WCSession / WorkoutCommunicationDelegate flows
        print("📱 [OutdoorHikeViewController] WorkoutCommunicationHandler didReceiveMessage: \(message)")
        // If needed, you can route messages into existing handling here.
    }
    
    func workoutCommunicationHandler(_ handler: WorkoutCommunicationHandler, didUpdateConnectionStatus isConnected: Bool) {
        DispatchQueue.main.async {
            print("📱 [OutdoorHikeViewController] Watch connection status changed: \(isConnected)")
...
extension OutdoorHikeViewController {
    ...
    func getHikingLogs(completion: @escaping ([HikeLog]?, Error?) -> Void) {
        ...
        guard let userId = UserIDResolver.shared.getBestUserIdForAPI(), !userId.isEmpty else {
            print("❌ [OutdoorHikeViewController] No user ID available")
            completion(nil, NSError(domain: "OutdoorHikeViewController", code: -1, userInfo: [NSLocalizedDescriptionKey: "User not authenticated"]))
            return
        }
        
        print("📥 [OutdoorHikeViewController] Fetching hiking logs for user: \(userId)")
            ...
            ActivityService.shared.getHikes(
                ...
                case .success(let response):
                    guard let data = response.data else {
                        print("❌ [OutdoorHikeViewController] No data in response")
                        completion(nil, NSError(domain: "OutdoorHikeViewController", code: -1, userInfo: [NSLocalizedDescriptionKey: "No data in response"]))
                        return
                    }
                    ...
                    print("📄 [OutdoorHikeViewController] Fetched page with \(pageLogs.count) hikes (Total: \(allHikeLogs.count))")
                    
                    // Check if there are more pages
                    if data.hasMore, let token = data.nextToken {
                        print("📄 [OutdoorHikeViewController] Has more pages, fetching next...")
                        fetchPage(nextToken: token)
                    } else {
                        print("✅ [OutdoorHikeViewController] Fetched all hikes: \(allHikeLogs.count) total")
                ...
                case .failure(let error):
                    print("❌ [OutdoorHikeViewController] Error fetching hikes: \(error.localizedDescription)")
        ...
        if let activityDataString = activity.activityData,
           let data = activityDataString.data(using: .utf8) {
            do {
                ...
                print("⚠️ [OutdoorHikeViewController] Failed to parse activityData: \(error)")
            }
        }
...
extension OutdoorHikeViewController: WCSessionDelegate {
    ...
    func sessionReachabilityDidChange(_ session: WCSession) {
        DispatchQueue.main.async { [weak self] in
            ...
            // If watch becomes unreachable, ask HikeTrackingEngine to evaluate primary sources
...
func session(_ session: WCSession, didReceiveMessage message: [String: Any], replyHandler: @escaping ([String: Any]) -> Void) {
    ...
    DispatchQueue.main.async { [weak self] in
        guard let self = self else { 
            ...
            print("📱 OutdoorHikeViewController: Received message with reply handler: \(message)")
            
            // Handle hike history request for pace zone calculation
            if let type = message["type"] as? String, type == "requestHikeHistory" {
                print("📱 🚀 OutdoorHikeViewController: HANDLING HIKE HISTORY REQUEST")
            let limit = message["limit"] as? Int ?? 20
                self.handleHikeHistoryRequest(limit: limit, replyHandler: replyHandler)
            return
        }
        
            // Check if this is an outdoor hike state change command - forward to HikeTrackingEngine
        if let type = message["type"] as? String, type == "outdoorHikeStateChange" {
                print("📱 OutdoorHikeViewController: Forwarding outdoorHikeStateChange message to HikeTrackingEngine")
            
            // Forward to HikeTrackingEngine's message handler
            self.engine.session(session, didReceiveMessage: message, replyHandler: replyHandler)
            return
        }
        
            // Handle other message types that OutdoorHikeViewController should process
  ...
  func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
    ...
    DispatchQueue.main.async { [weak self] in
        ...
        if let type = applicationContext["type"] as? String, type == "outdoorHikeStateChange" {
                print("📱 OutdoorHikeViewController: Forwarding outdoorHikeStateChange from application context to HikeTrackingEngine")
            // Forward to HikeTrackingEngine's application context handler
            self.engine.session(session, didReceiveApplicationContext: applicationContext)
            return
        }
    ...
    public func handleWatchStatus(_ status: [String: Any]) {
        ...
        if let isWatchTracking = status["isTracking"] as? Bool {
            engine.isWatchTracking = isWatchTracking
    ...
    /// Handle hike history request from watch for pace zone calculation
    public func handleHikeHistoryRequest(limit: Int, replyHandler: @escaping ([String: Any]) -> Void) {
        print("📱 🚀 OutdoorHikeViewController: STARTING HIKE HISTORY REQUEST HANDLER")
        print("📱 📊 Request limit: \(limit)")
        
        // Fetch hike history from server
        let group = DispatchGroup()
        var hikeLogs: [Any] = []
        ...
        // Fetch hike logs
        group.enter()
        print("📱 🔄 OutdoorHikeViewController: Starting hike logs fetch...")
        getHikingLogs { [weak self] logs, error in
            defer { group.leave() }
            
            if let error = error {
                print("📱 ❌ OutdoorHikeViewController: Error fetching hike logs: \(error.localizedDescription)")
                fetchErrors.append(error)
            } else if let logs = logs {
                hikeLogs = logs
                print("📱 ✅ OutdoorHikeViewController: Successfully fetched \(logs.count) hike logs")
            } else {
                print("📱 ❌ OutdoorHikeViewController: Failed to fetch hike logs - no data returned")
                fetchErrors.append(NSError(domain: "OutdoorHikeViewController", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to fetch hike logs"]))
            }
        }
        
        // Wait for request to complete
        group.notify(queue: .main) {
            print("📱 🔄 OutdoorHikeViewController: DATABASE REQUEST COMPLETED")
            print("📱 📊 FINAL RESULTS:")
            print("�� 📊 - Hike logs: \(hikeLogs.count)")
            print("�� �� - Fetch errors: \(fetchErrors.count)")
            
            // If we have errors but no data, return error
            if !fetchErrors.isEmpty && hikeLogs.isEmpty {
                let errorMessage = fetchErrors.first?.localizedDescription ?? "Unknown error"
                print("📱 ❌ OutdoorHikeViewController: NO DATA FETCHED AND ERRORS OCCURRED - Returning error")
                replyHandler(["status": "error", "message": errorMessage])
                return
            }
            
            // Process and send results
            var allHikes: [[String: Any]] = []
            
            // Convert hike logs to dictionary format
            for hike in hikeLogs {
                if let hikeDict = hike as? [String: Any] {
                    var processedHike = hikeDict
                    processedHike["hikeType"] = self.hikeType.rawValue
                    allHikes.append(processedHike)
                }
            }
            
            // Sort by creation date (most recent first) and limit results
            let sortedHikes = allHikes.sorted { (first, second) in
                let firstDate = first["createdAt"] as? Date ?? Date.distantPast
                let secondDate = second["createdAt"] as? Date ?? Date.distantPast
                return firstDate > secondDate
            }
            
            let limitedHikes = Array(sortedHikes.prefix(limit))
            
            let response: [String: Any] = [
                "status": "success",
                "hikeLogs": limitedHikes,
                "count": limitedHikes.count,
                "totalCount": hikeLogs.count,
                "timestamp": Date().timeIntervalSince1970
            ]
            
            print("📱 🚀 OutdoorHikeViewController: SENDING FINAL RESPONSE TO WATCH")
            print("📱 📦 Response keys: \(response.keys.joined(separator: ", "))")
            print("📱 �� Sending \(limitedHikes.count) hike logs to watch")
    ...
    public func sendActiveWorkoutToWatch(replyTo message: [String: Any]? = nil, directReplyHandler: (([String: Any]) -> Void)? = nil) {
        ...
        if engine.hikeState != .notStarted {
            ...
            response["workoutType"] = hikeType.rawValue
            response["hikeState"] = engine.hikeState.rawValue
            response["elapsedTime"] = engine.elapsedTime
            ...
            response["primaryMetricSources"] = [
                "distance": engine.isWatchTracking ? "watch" : "phone",
                "pace": engine.isWatchTracking ? "watch" : "phone",
                "hr": engine.isWatchTracking ? "watch" : "phone",
                "calories": engine.isWatchTracking ? "watch" : "phone"
            ]
        } else {
    ...
    public func prepareWatchResponse() -> [String: Any] {
        ...
        // Add current status
        response["isRunning"] = engine.hikeState == .inProgress
        response["elapsedTime"] = engine.elapsedTime
    ...
    func receivedWorkoutRequest(replyHandler: @escaping ([String : Any]) -> Void) {
        print("📱 OutdoorHikeViewController: Received workout request from watch")
        
        let isRunning = engine.hikeState == .inProgress || engine.hikeState == .paused
        
        // Debug current state
        print("📱 OutdoorHikeViewController DEBUG:")
        print("   - engine.hikeType: \(engine.hikeType)")
        print("   - engine.hikeState: \(engine.hikeState)")
        print("   - engine.isIndoorMode: \(engine.isIndoorMode)")
        ...
        var response: [String: Any] = [
            ...
            "workoutType": "hiking",  // Set hiking type
            "hikeType": engine.hikeType.rawValue,  // Set specific hike type
            "isIndoor": false             // Add explicit outdoor flag
        ]
        
        // Add metrics if running
        if isRunning {
            response["distance"] = engine.distance.value
            response["elapsedTime"] = engine.elapsedTime
            response["pace"] = engine.pace.value
            response["calories"] = engine.calories
            response["heartRate"] = engine.heartRate
        }
        
        print("📱 OutdoorHikeViewController: Sending workout status to watch: \(response)")
        replyHandler(response)
    }
    // ... existing code ...
}
...
extension OutdoorHikeViewController: AVSpeechSynthesizerDelegate {
        ...
        if let watchElapsedTime = metrics["elapsedTime"] as? TimeInterval, 
           watchElapsedTime > 0 {
            ...
            if timeDiff > 2.0 {
                ...
                // Update engine time
                if watchElapsedTime > engine.elapsedTime {
                    engine.elapsedTime = watchElapsedTime
                }
...
// Add an extension to HikeTrackingEngine with the missing method
...
extension OutdoorHikeViewController: WorkoutCommunicationDelegate {
    func didReceiveRunningWorkoutUpdate(_ update: [String: Any]) {
           // Not applicable for hiking, but required by protocol
           print("📱 Received running workout update (not applicable for hiking)")
       }
    ...
    func didReceiveHikingWorkoutUpdate(_ update: [String: Any]) {
        ...
        if let distance = update["distance"] as? Double {
            ...
            engine.updateDistanceFromWatch(distance)
        }
        ...
        if let calories = update["calories"] as? Double {
            ...
            engine.updateCalories(calories)
        }
    ...
    func currentWorkoutMetrics() -> [String: Any] {
        ...
        let isAutoJoin = engine.isJoiningExistingWorkout
        
        // For completed state or auto-join, include all final metrics
        if engine.hikeState == .completed || isAutoJoin {
            metrics = [
                "distance": engine.distance.value,
                "pace": engine.pace.value,
                "heartRate": engine.heartRate,
            ...
            if !handler.isPrimaryForDistance {
                metrics["distance"] = engine.distance.value
            }
            if !handler.isPrimaryForPace {
                metrics["pace"] = engine.pace.value
            }
            if !handler.isPrimaryForHeartRate {
                metrics["heartRate"] = engine.heartRate
            }
            if !handler.isPrimaryForCalories {
                metrics["calories"] = engine.calories
            }
            if !handler.isPrimaryForCadence {
                metrics["cadence"] = engine.cadence
            }
            
            // Always include elapsed time
            metrics["elapsedTime"] = engine.elapsedTime
            
            // Set workout status flags
            metrics["workoutActive"] = engine.hikeState == .inProgress || engine.hikeState == .paused
            metrics["hasActiveWorkout"] = engine.hikeState != .notStarted
        }
        
        // Always include these metadata fields regardless of state
        metrics["hikeState"] = engine.hikeState.rawValue
        metrics["state"] = engine.hikeState.rawValue // Include both for compatibility
        metrics["isIndoor"] = false // Hikes are always outdoor
        metrics["hikeType"] = "outdoorRun"
        metrics["workoutType"] = "run"
        metrics["timestamp"] = Date().timeIntervalSince1970
        metrics["id"] = engine.workoutId.uuidString
        ...
        metrics["hasGoodLocationData"] = engine.hasGoodLocationData
        metrics["useImperialUnits"] = !engine.useMetric
        
        return metrics
    }
    
    // Removed typed HikingWorkout overload; use dictionary-based updates via didReceiveHikingWorkoutUpdate(_:) instead
...
extension OutdoorHikeTrackerView {
    ...
    /// Gets comprehensive performance coaching with recovery and fatigue analysis
       ...
       public func getComprehensiveCoaching() -> String? {
           let currentPace = viewModel.engine.pace.value
           let heartRate = viewModel.engine.heartRate
           let elapsedTime = viewModel.engine.elapsedTime
           let distance = viewModel.engine.distance.value
           
           // Early data check
           guard currentPace > 0 && !currentPace.isInfinite else {
            return "🔥 Getting warmed up! Your hiking data is loading..."
           }
    ...
    // MARK: - Coaching Methods (Hiking)
    
    /// Heart-rate based status/coaching text for hiking
    public func analyzeHeartRateStatus() -> String? {
        ...
        switch pct {
        case 0..<50:  return "💙 Cruising in the comfort zone! Perfect for recovery hikes."
        case 50..<60: return "💚 Aerobic zone! Building endurance steadily on the trail."
        case 60..<70: return "💛 Strong aerobic work! Nice and sustainable hiking pace."
        case 70..<80: return "🧡 Threshold work! Great effort—stay controlled on the ascent."
        case 80..<90: return "❤️ Very hard effort. Consider easing soon on steep sections."
        case 90..<95: return "🔴 Near redline—back off to stay safe on the trail."
        default:      return nil // will trigger critical warning
        }
    }
    
    /// Pace coaching with simple target comparison for hiking
    public func analyzePacePerformance() -> String? {
        guard let targetPace = viewModel.targetPace else {
            return "🎯 Going by feel today—listen to your body on the trail."
        }
        let current = viewModel.engine.pace.value
        let diff = current - targetPace
        let tol = targetPace * 0.05
        if abs(diff) <= tol {
            let msgs = [
                "🎯 Pace on point—keep the rhythm on the trail!",
                "✨ Smooth and steady—nice hiking pace!",
                "🔥 Consistent pacing—great work on varied terrain!"
            ]
            return msgs.randomElement()
        }
        if diff > tol {
            let slowBy = Int((diff / targetPace) * 100.0)
            return slowBy > 15 ? "🐌 \(slowBy)% slower than target—add a little spark on the trail!" : "🚶‍♂️ Slightly behind—gradually pick it up on the ascent."
        } else {
            let fastBy = Int((abs(diff) / targetPace) * 100.0)
            return fastBy > 15 ? "🚀 \(fastBy)% faster—save some energy for the summit!" : "🏃‍♂️ A tad fast—stay in control on the descent."
        }
    }
    
    /// Safety: warn when HR extremely high during hiking
    public func getCriticalHealthWarning() -> String? {
        let hr = viewModel.engine.heartRate
        guard hr > 0 else { return nil }
        let maxHR = 220.0 - 30.0
        let pct = (hr / maxHR) * 100.0
        if pct >= 95.0 {
            return "🚨 SLOW DOWN! Heart rate dangerously high—take a recovery break on the trail."
        }
        return nil
    }
    
    /// High fatigue advice for hiking
    public func getFatigueWarning() -> String? {
        let msgs = [
            "😴 Fatigue detected—consider a short recovery break on the trail.",
            "⚡ Working hard—ease up if needed on steep sections.",
            "🔋 Energy dipping—hike recovery can help maintain pace.",
            "💪 Pushing limits—remember, rest builds strength for the summit."
        ]
        return msgs.randomElement()
    }
    
    /// Recovery tips based on duration/distance/HR for hiking
    public func getRecoveryAdvice() -> String? {
        let mins = viewModel.engine.elapsedTime / 60.0
        let dist = viewModel.engine.distance.value
        if mins > 45 || dist > 5_000.0 { // meters
            let tips = [
                "🥤 Hydrate regularly on longer hikes.",
                "🧂 Consider electrolytes for extended effort on the trail.",
                "🍌 Bring a small snack for energy on long hikes.",
                "❄️ Plan post-hike recovery: light stretching, cooldown."
            ]
        ...
        if hr > 0 {
            let maxHR = 220.0 - 30.0
            let pct = (hr / maxHR) * 100.0
            if pct > 80 && mins > 20 { return "🌡️ Hot day or high effort—cool down gradually on the trail." }
        }
        return nil
    }
    
    /// Encouragement messages for hiking
    public func getEncouragementMessage() -> String? {
        let elapsedMinutes = viewModel.engine.elapsedTime / 60.0
        let distanceMeters = viewModel.engine.distance.value
        
        // Distance-based
        if distanceMeters >= (viewModel.engine.useMetric ? 5000.0 : 1609.34 * 3.1) {
            return [
                "🚶‍♂️ 5K+—fantastic hiking effort!",
                "💪 Strong hike! Every step on the trail counts!",
                "🎉 Crushing it today—keep going toward the summit!"
            ].randomElement()
        }
        // Time-based
        if elapsedMinutes >= 30 {
            return [
                "⏰ 30+ minutes—great hiking consistency!",
                "🔥 Half-hour mark—nice work on the trail!",
                "✨ Time flies when you're exploring nature!"
            ].randomElement()
        }
        // General
        return [
            "🌟 Every step builds momentum on the trail!",
            "💚 Feel the energy—keep it steady on the ascent!",
            "🔥 Progress over perfection on the hiking path!"
        ].randomElement()
       }
    ...
    public func analyzeFatigueLevel() -> Double {
        ...
        // Heart rate trend analysis (35% weight)
        if heartRateHistory.count >= 10 {
            let recentHR = Array(heartRateHistory.suffix(5))
            let earlyHR = Array(heartRateHistory.prefix(5))
            
            if !earlyHR.isEmpty && !recentHR.isEmpty {
                ...
                if avgEarlyHR > 0 {
                    ...
                    fatigueScore += max(0, hrIncrease * 1.5) * 0.35 // Higher HR = higher fatigue
                }
            }
        }
        
        // Time-based fatigue (25% weight)
        let timeFatigue = min(1.0, elapsedMinutes / 120.0) // Max fatigue after 2 hours
        fatigueScore += timeFatigue * 0.25
        
        // Normalize score to 0-100 range
        let normalizedScore = min(100.0, fatigueScore * 100.0)
        
        return normalizedScore
    }
}

// MARK: - Hiking Performance Components

struct HikePerformanceCard: View {
    @Binding var showPerformance: Bool
    let cardBackground: Color
    let cardCornerRadius: CGFloat
    let engine: HikeTrackingEngine
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if showPerformance {
                VStack(spacing: 20) {
                    HikeMetricsView(engine: engine)
                    HikePaceZoneView(engine: engine, targetPace: targetPace, targetPaceSource: targetPaceSource)
                    HikeHeartRateZoneView(engine: engine)
                    HikeEffortScoreView(engine: engine)
                    HikeSplitTimesView(engine: engine)
                }
                .padding(.bottom, 16)
        ...
        .onTapGesture { withAnimation(.spring(response: 0.5, dampingFraction: 0.7)) { showPerformance.toggle() } ; UIImpactFeedbackGenerator(style: .light).impactOccurred() }
        .onLongPressGesture(minimumDuration: 0, maximumDistance: .infinity, pressing: { pressing in withAnimation(.easeInOut(duration: 0.1)) { isPressed = pressing } }, perform: {})
    }
    
    var performanceHeader: some View {
        HStack(spacing: 12) {
            ZStack { Circle().fill(LinearGradient(colors: [Color.blue.opacity(0.8), Color.purple.opacity(0.6)], startPoint: .topLeading, endPoint: .bottomTrailing)).frame(width: 32, height: 32); Image(systemName: "chart.line.uptrend.xyaxis").font(.system(size: 14, weight: .semibold)).foregroundColor(.white) }
            VStack(alignment: .leading, spacing: 2) { Text("PERFORMANCE").font(.system(size: 12, weight: .bold)).foregroundColor(.white).tracking(0.5); Text(showPerformance ? "Tap to collapse" : "Tap to expand").font(.system(size: 10, weight: .medium)).foregroundColor(.gray).opacity(0.8) }
            Spacer()
            Image(systemName: showPerformance ? "chevron.up" : "chevron.down").font(.system(size: 14, weight: .semibold)).foregroundColor(.white).rotationEffect(.degrees(showPerformance ? 180 : 0)).animation(.spring(response: 0.4, dampingFraction: 0.8), value: showPerformance)
        }
        .padding(.horizontal, 20)
        .padding(.vertical, 16)
...
// Hike Split Times View Component
struct HikeSplitTimesView: View {
    let engine: HikeTrackingEngine
    
    private var splitUnit: String {
        return engine.useMetric ? "km" : "mi"
    }
    ...
    // Precomputed values to keep the view builder simple
    private var distanceValue: Double { engine.distance.value }
    private var completedSplits: Int { Int(distanceValue / splitUnitDistance) }
    private var currentProgressPercentText: String {
        let currentProgress = (distanceValue / splitUnitDistance) - Double(completedSplits)
        return String(format: "%.0f%%", currentProgress * 100)
    }
    // Starting index for the last N splits to render human-friendly numbering
    private var splitStartIndex: Int { max(engine.splitTimes.count - splitsToShow.count, 0) }
    
    // Lightweight strings and rows to keep body simple
    private var splitsHeader: String { "SPLITS (\(splitUnit.uppercased()))" }
    private struct SplitRow: Identifiable { let id: Int; let label: String; let pace: String }
    private var splitRows: [SplitRow] {
        let start = splitStartIndex + 1
        return splitsToShow.enumerated().map { (i, s) in
            SplitRow(id: start + i, label: "Split \(start + i)", pace: formatSplitPace(s))
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Text(currentProgressPercentText)
                    .font(.footnote)
                    .foregroundColor(.white)
            }
...
// Hiking Pace Zone View
struct HikePaceZoneView: View {
    @ObservedObject var engine: HikeTrackingEngine
    let targetPace: Double?
    let targetPaceSource: String
    ...
    func zoneLabel(_ text: String, color: Color) -> some View {
        Text(text).font(.system(size: 10, weight: .medium)).foregroundColor(color.opacity(0.8))
    }
    ...
    func calculatePacePosition() -> CGFloat {
        ...
        let excess = currentPace - slow, maxExcess = targetPace * 0.35
        let slowPos = 1.0 - min(excess / maxExcess, 1.0)
        return slowPos * 0.25
    }
    
    func formatPace(_ secs: Double) -> String { String(format: "%d:%02d", Int(secs)/60, Int(secs)%60) }
}

// Hiking Heart Rate Zone View
struct HikeHeartRateZoneView: View {
    let engine: HikeTrackingEngine
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Image(systemName: "heart.fill").font(.system(size: 14, weight: .semibold)).foregroundColor(.red)
                Text("HEART RATE ZONE").font(.system(size: 12, weight: .bold)).foregroundColor(.white).tracking(0.5)
                Spacer()
                Text("\(Int(engine.heartRate)) bpm").font(.system(size: 14, weight: .semibold)).foregroundColor(.red)
            ...
            VStack(spacing: 12) {
                heartRateZoneGraph
                HStack { zoneLabel("Rest", color: .blue); Spacer(); zoneLabel("Fat Burn", color: .green); Spacer(); zoneLabel("Cardio", color: .orange); Spacer(); zoneLabel("Peak", color: .red) }
                    .padding(.horizontal, 20)
            }
        }
    }
    ...
    var heartRateZoneGraph: some View {
        GeometryReader { metrics in
            ZStack(alignment: .leading) {
                HStack(spacing: 2) {
                    Rectangle().fill(LinearGradient(colors: [Color.blue.opacity(0.8), Color.blue.opacity(0.5)], startPoint: .top, endPoint: .bottom)).frame(width: (metrics.size.width - 6) * 0.25)
                    Rectangle().fill(LinearGradient(colors: [Color.green.opacity(0.8), Color.green.opacity(0.5)], startPoint: .top, endPoint: .bottom)).frame(width: (metrics.size.width - 6) * 0.25)
                    Rectangle().fill(LinearGradient(colors: [Color.orange.opacity(0.8), Color.orange.opacity(0.5)], startPoint: .top, endPoint: .bottom)).frame(width: (metrics.size.width - 6) * 0.25)
                    Rectangle().fill(LinearGradient(colors: [Color.red.opacity(0.8), Color.red.opacity(0.5)], startPoint: .top, endPoint: .bottom)).frame(width: (metrics.size.width - 6) * 0.25)
                }
                .frame(height: 24).cornerRadius(12)
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.white.opacity(0.1), lineWidth: 1))
                
                VStack {
                    Circle().fill(Color.white).frame(width: 8, height: 8).shadow(color: .red, radius: 4)
                    Rectangle().fill(Color.white).frame(width: 2, height: 20).cornerRadius(1)
                }
    ...
    func calculateHeartRatePosition() -> CGFloat {
        ...
        let minHR = 50.0, maxHR = 180.0
        let norm = (currentHR - minHR) / (maxHR - minHR)
        return CGFloat(min(max(norm, 0.0), 1.0))
    }
}

// Hiking Effort Score
struct HikeEffortScoreView: View {
    let engine: HikeTrackingEngine
    var effortScore: Int {
        let hr = engine.heartRate
        let dur = engine.elapsedTime / 60.0
        guard hr > 0, dur > 0 else { return 0 }
        let maxHR = 220.0 - 30.0
        var base = Int(min(100, max(0, (hr / maxHR) * 100)))
        let factor = min(2.0, 1.0 + (dur / 60.0))
        base = Int(Double(base) * factor * 0.8)
        return min(100, base)
    }
    var effortLevel: String { let s = effortScore; return s < 20 ? "Very Low" : s < 40 ? "Low" : s < 60 ? "Moderate" : s < 80 ? "High" : "Elite" }
    var effortColor: Color { let s = effortScore; return s < 20 ? .blue : s < 40 ? .green : s < 60 ? .yellow : s < 80 ? .orange : .red }
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack { Text("EFFORT SCORE").font(.footnote).foregroundColor(.gray); Spacer(); Text("\(effortScore)").font(.footnote).foregroundColor(.white) }
            ZStack(alignment: .leading) {
                Rectangle().fill(Color.gray.opacity(0.3)).frame(height: 30).cornerRadius(8)
                Rectangle().fill(effortColor.opacity(0.8)).frame(width: max(CGFloat(effortScore)/100.0 * UIScreen.main.bounds.width - 32, 4), height: 30).cornerRadius(8)
                Text(effortLevel).font(.footnote).fontWeight(.medium).foregroundColor(.white).padding(.leading, 12)
            }
        }
        .padding(.horizontal, 16)
    }
...
// Hiking Metrics
struct HikeMetricsView: View {
    let engine: HikeTrackingEngine
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Image(systemName: "figure.hiking").font(.system(size: 14, weight: .semibold)).foregroundColor(.blue)
                Text("HIKING METRICS").font(.system(size: 12, weight: .bold)).foregroundColor(.white).tracking(0.5)
                Spacer()
            }
            .padding(.horizontal, 20)
            HStack(spacing: 16) {
...
// Hike-specific delegate to avoid ambiguity
protocol OutdoorHikeViewControllerDelegate: AnyObject {
    func outdoorHikeDidComplete(with hikeLog: HikeLog?)
    func outdoorHikeWasCanceled()
}

// Add this after the existing structs, before the end of the file
struct VoiceGuidanceHikeSettingsView: View {
    @ObservedObject var viewModel: OutdoorHikeViewController
            ...
            Form {
                Section(header: Text("Voice Guidance Level")) {
                    Picker("Level", selection: $viewModel.voiceGuidanceLevel) {
                        ForEach(OutdoorHikeViewController.VoiceGuidanceLevel.allCases, id: \.self) { level in
                            Text(level.rawValue)
                                .tag(level)
                        }
                    }
                ...
                Section(header: Text("Announcement Frequency")) {
                    Picker("Frequency", selection: $viewModel.announcementFrequency) {
                        ForEach(OutdoorHikeViewController.AnnouncementFrequency.allCases, id: \.self) { frequency in
