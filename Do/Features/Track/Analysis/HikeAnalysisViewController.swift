...
class HikeAnalysisViewController: UIViewController {
    private var hostingController: UIHostingController<HikeAnalysisView>?
    var hike: HikeLog?
    ...
    private func setupHostingController() {
        ...
        let analysisView = HikeAnalysisView(hike: hike, onDismiss: { [weak self] in self?.dismiss(animated: true) })
...
struct HikeAnalysisView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showShareSheet) {
            RunShareTemplateHub(
                run: hike,
                isOutdoorRun: !routeLocations.isEmpty,
    ...
    private var summaryCardsView: some View {
        VStack(spacing: 16) {
            ...
            HStack(spacing: 8) {
                metricCard(title: "DISTANCE", value: getDistance(), icon: "figure.hiking", color: Color(hex: 0x4CD964))
    ...
    private var overviewView: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // Hike Performance Metrics
                HStack(spacing: 12) {
                    metricCard(title: "Elevation Gain", value: hike.elevationGain ?? "--", icon: "mountain.2", color: Color(hex: 0x4CD964))
                    metricCard(title: "Max Elevation", value: formatMaxElevation(), icon: "arrow.up.circle", color: Color(hex: 0xFF3B30))
                }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("SCORING GUIDE")
                    ...
                    .foregroundColor(.white.opacity(0.7))
                    .padding(.top, 4)
                Text("• Pace: Consistency of your speed during the hike")
                    .font(.system(size: 9))
                    .foregroundColor(.white.opacity(0.7))
                ...
                Text("• Endurance: Duration relative to your history")
                    .font(.system(size: 9))
                    .foregroundColor(.white.opacity(0.7))
                Text("• Elevation: Challenge from elevation gain/loss")
                    .font(.system(size: 9))
                    .foregroundColor(.white.opacity(0.7))
    ...
    // MARK: - Performance Score Calculations (Hike)
    ...
    private func getPerformanceMetrics() -> [PerformanceMetric] {
        ...
        // Heart Rate zone (if data exists)
        if let hrScore = calculateHeartRateZoneScoreHike() {
            metrics.append(PerformanceMetric(name: "HEART RATE", score: hrScore, icon: "heart.fill", color: Color(hex: 0xFF375F)))
        }
        // Endurance (duration)
        metrics.append(PerformanceMetric(name: "ENDURANCE", score: calculateDurationScoreHike(), icon: "clock", color: Color(hex: 0xFF9500)))
        // Elevation challenge
        metrics.append(PerformanceMetric(name: "ELEVATION", score: calculateElevationScore(), icon: "arrow.up.right", color: Color(hex: 0x5856D6)))
        return metrics
    }
    private func calculateHeartRateZoneScoreHike() -> Double? {
        let data = extractHeartRateDataHike()
    ...
    private func extractHeartRateDataHike() -> [Double] {
        if let locs = hike.locationData {
            return locs.compactMap { $0["heartRate"] as? Double }.filter { $0 > 0 }
        }
        return []
    }
    private func calculateDurationScoreHike() -> Double {
        let target = 90.0 // minutes
        var actual = 0.0
        if let d = hike.duration { actual = convertDurationToMinutesHike(d) }
        let raw = min(actual / target * 100, 100)
        let bonus = actual > target ? min((actual - target) / 30 * 10, 20) : 0
        return min(100, raw + bonus)
    }
    private func convertDurationToMinutesHike(_ durationStr: String) -> Double {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .bottom) {
                    RoutePositionStatsCard(routeLocations: routeLocations, run: convertHikeToRunProxy(), progress: $routeProgress, isPlaying: $isPlayingRoute)
                        .padding(.bottom, 66)
                    RouteSeekSlider(value: $routeProgress, isPlaying: $isPlayingRoute, run: convertHikeToRunProxy(), onPlayPause: { isPlaying in
    ...
    private func convertHikeToRunProxy() -> Any { hike }
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                HStack(spacing: 12) {
                    ...
                    statCard(title: "MAX ELEV", value: formatMaxElevation(), icon: "arrow.up.circle", color: Color(hex: 0xAF52DE))
                }
            }
            .padding(16)
        }
            ...
            VStack(spacing: 20) {
                ...
                if !routeLocations.isEmpty {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        let splits = calculateHikeSplits()
                        if !splits.isEmpty {
                            VStack(spacing: 0) {
                                // Header
                                HStack {
                                ...
                                ForEach(0..<splits.count, id: \.self) { index in
                                    ...
                                    HStack {
                                        ...
                                        Text(formatHikeSplitTime(split.time))
                                            .font(.system(size: 14))
                                            .foregroundColor(.white)
                                            .frame(width: 55, alignment: .leading)
        ...
        return Chart {
            if !paceDataPoints.isEmpty {
                ForEach(Array(paceDataPoints.enumerated()), id: \.offset) { index, point in
                    LineMark(x: .value("Distance", point.distance), y: .value("Pace", point.pace))
                    .foregroundStyle(LinearGradient(gradient: Gradient(colors: [Color(hex: 0x4CD964), Color(hex: 0x007AFF)]), startPoint: .leading, endPoint: .trailing))
                    .lineStyle(StrokeStyle(lineWidth: 3, lineCap: .round))
                    AreaMark(x: .value("Distance", point.distance), y: .value("Pace", point.pace))
                    .foregroundStyle(LinearGradient(gradient: Gradient(colors: [Color(hex: 0x4CD964).opacity(0.4), Color(hex: 0x007AFF).opacity(0.1)]), startPoint: .top, endPoint: .bottom))
                    if index % max(1, paceDataPoints.count / 6) == 0 { PointMark(x: .value("Distance", point.distance), y: .value("Pace", point.pace)).foregroundStyle(Color.white).symbolSize(30) }
                }
            }
        }
        .chartYAxis { AxisMarks(position: .leading) { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let pace = value.as(Double.self) { Text(formatPaceValue(pace)).font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } }
        .chartXAxis { AxisMarks { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let distance = value.as(Double.self) { Text(formatDistanceValue(distance)).font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } }
            ...
            if !routeLocations.isEmpty {
                ...
                Chart {
                    ForEach(Array(elevationData.enumerated()), id: \.offset) { index, elevation in
                        LineMark(x: .value("Distance", Double(index) * getChartDistanceInterval(elevationData.count)), y: .value("Elevation", elevation))
                        ...
                        .lineStyle(StrokeStyle(lineWidth: 3, lineCap: .round))
                        AreaMark(x: .value("Distance", Double(index) * getChartDistanceInterval(elevationData.count)), y: .value("Elevation", elevation))
                ...
                .chartYAxis { AxisMarks(position: .leading) { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let elevation = value.as(Double.self) { let useMetric = UserPreferences.shared.useMetricSystem; if useMetric { Text("\(Int(elevation))m").font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } else { let feet = elevation * 3.28084; Text("\(Int(feet))ft").font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } } }
                .chartXAxis { AxisMarks { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let distance = value.as(Double.self) { Text(formatDistanceValue(distance)).font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } }
            } else { Text("No elevation data available").foregroundColor(.white.opacity(0.7)).frame(maxWidth: .infinity, maxHeight: .infinity) }
        }
    }
        ...
        Group {
            if let locs = hike.locationData {
                ...
                if !heartRateData.isEmpty {
                    ...
                    .chartYAxis { AxisMarks(position: .leading) { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let v = value.as(Double.self) { Text("\(Int(v)) bpm").font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } }
                    .chartXAxis { AxisMarks { value in AxisGridLine(centered: true, stroke: StrokeStyle(lineWidth: 0.5, dash: [4])).foregroundStyle(Color.white.opacity(0.3)); AxisValueLabel() { if let t = value.as(Double.self) { Text(formatTimeValue(t)).font(.system(size: 10)).foregroundColor(.white.opacity(0.7)) } } } }
                } else { Text("No heart rate data available").foregroundColor(.white.opacity(0.7)) }
            } else { Text("No heart rate data available").foregroundColor(.white.opacity(0.7)) }
        }
    }

    // Helpers for charts (match Run)
    private func calculatePaceChartData() -> [PacePoint] {
        ...
        for i in 1..<locations.count { let d = locations[i].distance(from: locations[i-1]); let dt = max(0.1, locations[i].timestamp.timeIntervalSince(locations[i-1].timestamp)); cumDist.append(cumDist.last! + d); cumTime.append(cumTime.last! + dt) }
        ...
        func interpolateTime(_ target: Double) -> Double { if target <= 0 { return 0 }; var lo = 0, hi = cumDist.count - 1; while lo < hi { let mid = (lo + hi)/2; if cumDist[mid] < target { lo = mid + 1 } else { hi = mid } }; let idx = max(1, lo); let d0 = cumDist[idx-1], d1 = cumDist[idx]; let t0 = cumTime[idx-1], t1 = cumTime[idx]; if d1 <= d0 { return t1 }; let ratio = (target - d0)/(d1 - d0); return t0 + ratio*(t1 - t0) }
        var prev: Double = 0
        for b in 1...numBins { let boundary = min(Double(b) * binSizeMeters, totalDist); let t0 = interpolateTime(prev); let t1 = interpolateTime(boundary); let meters = boundary - prev; let seconds = max(0.1, t1 - t0); let mps = meters / seconds; let paceMinPerUnit = (unitMeters / mps) / 60.0; let capped = min(20.0, max(3.0, paceMinPerUnit)); let xUnits = boundary / unitMeters; points.append(PacePoint(distance: xUnits, pace: capped)); prev = boundary }
        if points.count > 4 { var smoothed: [PacePoint] = []; for i in 0..<points.count { let idxs = [max(0,i-2),max(0,i-1),i,min(points.count-1,i+1),min(points.count-1,i+2)]; let vals = idxs.map{points[$0].pace}.sorted(); let med = vals[vals.count/2]; smoothed.append(PacePoint(distance: points[i].distance, pace: med)) }; return smoothed }
        return points
    }
    ...
    private func getChartTimeInterval(_ pointCount: Int) -> Double { var totalMinutes: Double = 30.0; if let d = hike.duration { totalMinutes = convertDurationToMinutesHike(d) }; return max(0.5, totalMinutes/Double(max(1, pointCount-1))) }
    ...
    private func setupData() {
        // First, try to fetch route data from S3 if available
        if let routeDataUrl = hike.routeDataUrl, hike.locationData == nil {
            print("📥 Fetching hike route data from S3: \(routeDataUrl)")
            fetchRouteDataFromS3(url: routeDataUrl) { [self] locationData in
                if let locationData = locationData {
                    print("✅ Fetched \(locationData.count) hike location points from S3")
                    self.routeLocations = self.extractLocationData(from: locationData)
                    self.routePoints = self.routeLocations.map { RoutePoint(coordinate: $0.coordinate) }
                    if !self.routeLocations.isEmpty {
                        DispatchQueue.main.async {
                            self.setMapRegion()
                        }
                    }
                } else {
                    print("⚠️ Failed to fetch hike route data from S3")
                }
            }
            return
        }
        
        // Fallback to coordinateArray (legacy Parse format)
        if let coords = hike.coordinateArray, !coords.isEmpty {
    ...
    /// Fetch route data from S3 using signed URL
    private func fetchRouteDataFromS3(url: String, completion: @escaping ([[String: Any]]?) -> Void) {
        guard let downloadUrl = URL(string: url) else {
            print("❌ Invalid S3 URL: \(url)")
            completion(nil)
            return
        }
        ...
        URLSession.shared.dataTask(with: downloadUrl) { data, response, error in
            if let error = error {
                print("❌ Error fetching hike route data: \(error.localizedDescription)")
                completion(nil)
                return
            }
            
            guard let data = data else {
                print("❌ No data received from S3")
                completion(nil)
                return
            }
            ...
            do {
                ...
                // Try to parse directly first, then fallback to manual decompression
                var json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
                
                // Second try: Manual decompression if needed
                if json == nil, let decompressed = try? (data as NSData).decompressed(using: .zlib) as Data {
                    json = try? JSONSerialization.jsonObject(with: decompressed) as? [String: Any]
                }
                ...
                if let json = json,
                   let points = json["points"] as? [[String: Any]] {
                    ...
                    print("❌ Invalid JSON structure in hike route data")
                    DispatchQueue.main.async {
                        completion(nil)
                    }
                }
            } catch {
                print("❌ Error decompressing/parsing hike route data: \(error)")
                DispatchQueue.main.async {
                    completion(nil)
                }
            }
    ...
    private func extractLocationData(from locationData: [[String: Any]]) -> [CLLocation] {
        ...
        for dict in locationData {
            ...
            if let la = dict["latitude"] as? Double, let lo = dict["longitude"] as? Double {
                lat = la; lon = lo
            } else if let la = dict["lat"] as? Double, let lo = dict["lng"] as? Double {
                lat = la; lon = lo
            }
    ...
    // Compute max elevation from available data (prefer routeLocations, fallback to locationData altitudes if present)
    private func computeMaxElevationMeters() -> Double? {
        if !routeLocations.isEmpty {
            let maxAlt = routeLocations.map { $0.altitude }.max() ?? 0
            return maxAlt
        }
        // Fallback: parse from hike.locationData if it includes altitude
        if let locs = hike.locationData, !locs.isEmpty {
            let alts = locs.compactMap { dict -> Double? in
                if let alt = dict["altitude"] as? Double { return alt }
                if let altStr = dict["altitude"] as? String, let v = Double(altStr) { return v }
                return nil
            }
            return alts.max()
        }
        // Fallback: parse from coordinateArray if PFGeoPoint extension includes altitude via custom key
        if let coords = hike.coordinateArray, !coords.isEmpty {
            // PFGeoPoint default has latitude/longitude only; no altitude. Return nil.
            return nil
        }
        return nil
    }
    
    private func formatMaxElevation() -> String {
        guard let maxMeters = computeMaxElevationMeters() else { return "--" }
        // Show in meters; if you want feet for imperial users, convert: feet = meters * 3.28084
        if UserPreferences.shared.useMetricSystem {
            return String(format: "%.0f m", maxMeters)
        } else {
            let feet = maxMeters * 3.28084
            return String(format: "%.0f ft", feet)
        }
    }
    
    private func calculateDistanceScore() -> Double {
        guard let distanceStr = hike.distance else { return 50.0 }
        // Simple scoring based on distance - can be enhanced
        return min(100, 50 + 25)
    }
    
    private func calculateElevationScore() -> Double {
        guard let elevationGain = hike.elevationGain else { return 50.0 }
        // Simple scoring based on elevation gain - can be enhanced
        return min(100, 50 + 25)
    }
    ...
    // Chips for stats parity with Bike/Run
    private func caloriesChip() -> String {
        if let c = hike.caloriesBurned { return String(format: "%.0f kcal", c) }
        return "-- kcal"
    }
    private func heartRateChip() -> String { return "-- bpm" }
    private func elevationGainChip() -> String { hike.elevationGain ?? "--" }
    ...
    // MARK: - Formatters matching Bike
    private func getDistance() -> String {
        let useMetric = UserPreferences.shared.useMetricSystem
        let raw = (hike.distance ?? "").replacingOccurrences(of: "[^0-9\\.]", with: "", options: .regularExpression)
        let km = Double(raw) ?? 0
        let value = useMetric ? km : km/1.60934
        return String(format: useMetric ? "%.2f km" : "%.2f mi", value)
    }
    private func getDuration() -> String { hike.duration ?? "--:--" }
    private func getAvgPaceFromHistory() -> String {
        if let p = hike.avgPace, !p.isEmpty { return p }
        // Fallback compute from distance and duration
        let raw = (hike.distance ?? "").replacingOccurrences(of: "[^0-9\\.]", with: "", options: .regularExpression)
        let km = Double(raw) ?? 0
        let secs: Double = {
            guard let d = hike.duration else { return 0 }
    ...
    private func formatDate() -> String {
        guard let date = hike.createdAt else { return "" }
    ...
    private func calculateHikeSplits() -> [HikeSplit] {
        guard routeLocations.count > 1 else { return [] }
    ...
    private func formatHikeSplitTime(_ seconds: Double) -> String {
