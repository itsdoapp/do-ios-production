...
//  Do.
//
//  Created by Mikiyas Meseret on 8/4/25.
...
// BikeType is defined in Features/Track/Models/BikingTypes.swift
// SplitTime, LocationData, RouteAnnotation, etc. are defined in Features/Track/Models/TrackingModels.swift
// BikeDataPoint is defined in Features/Track/Models/ActivityLogModels.swift

// Direct implementation of watch connectivity to avoid dependencies
...
extension Notification.Name {
    static let bikeTrackingEngineDidUpdateState = Notification.Name("bikeTrackingEngineDidUpdateState")
    static let bikeTrackingEngineDidUpdateMetrics = Notification.Name("bikeTrackingEngineDidUpdateMetrics")
    static let bikeTrackingEngineDidUpdateLocation = Notification.Name("bikeTrackingEngineDidUpdateLocation")
    static let bikeTrackingEngineDidUpdateBikeType = Notification.Name("bikeTrackingEngineDidUpdateBikeType")
    static let bikeSettingsDidChange = Notification.Name("bikeSettingsDidChange")
}

// MARK: - Bike-specific Enums and Structs

/// Enum representing the state of a bike workout
enum BikeState {
    case notStarted
    case active
    case paused
...
/// Enum representing the weather condition for biking
enum BikeWeatherCondition: String {
    case clear = "Clear"
    case cloudy = "Cloudy"
    case rainy = "Rainy"
    case snowy = "Snowy"
    ...
    var isGoodForBiking: Bool {
        switch self {
        case .clear, .cloudy, .windy:
            return true
        case .rainy, .snowy, .stormy, .foggy, .unknown:
            return false
        }
    }
}

/// Structure to hold weather data for biking
struct BikeWeatherData {
    let temperature: Double
    let condition: BikeWeatherCondition
...
// MARK: - Bike Tracking Engine Delegate

/// Protocol for receiving updates from the BikeTrackingEngine
protocol BikeTrackingEngineDelegate: AnyObject {
    /// Called when the tracking engine's state is updated
    func bikeTrackingEngineDidUpdateState(_ engine: BikeTrackingEngine)
    func bikeStateDidChange(_ state: BikeState)
    func bikeDistanceDidChange(_ distance: Double)
    func bikeSpeedDidChange(_ speed: Double)
    func bikeCaloriesDidChange(_ calories: Double)
    func bikeLocationDidChange(_ location: CLLocation)
    func bikeRouteDidChange(_ route: [CLLocation])
    func bikeWeatherDidChange(_ weather: BikeWeatherData)
    func bikeElapsedTimeDidChange(_ time: TimeInterval)
}

// Provide default implementations to make all methods optional
extension BikeTrackingEngineDelegate {
    func bikeTrackingEngineDidUpdateState(_ engine: BikeTrackingEngine) {}
    func bikeStateDidChange(_ state: BikeState) {}
    func bikeDistanceDidChange(_ distance: Double) {}
    func bikeSpeedDidChange(_ speed: Double) {}
...
class BikeTrackingEngine: NSObject, ObservableObject, WCSessionDelegate {
    // MARK: - Singleton Instance
    static let shared = BikeTrackingEngine()
    
    // MARK: - Published Properties
    
    /// Delegate to receive updates about the bike tracking engine state
    /// Note: The BikeTrackingEngineDelegate protocol is defined above (lines 132-153)
    weak var delegate: BikeTrackingEngineDelegate?
    ...
    @Published var bikeState: BikeState = .notStarted {
        didSet {
    ...
    @Published var bikeType: BikeType = .outdoorBike {
        didSet {
            updateBikeType(bikeType)
        }
    }
    ...
    private var bikeDataPoints: [BikeDataPoint]? = nil // For indoor bike tracking
    ...
    // Bike-specific alias to support callers expecting bikingEfficiency
    @Published var bikingEfficiency: Double = 0 // 0-100% scale
    ...
    private var bikeId: String? // AWS bike activity ID
    ...
    var isRunning: Bool { bikeState == .active }
    var isPaused: Bool { bikeState == .paused }
    
    // Unified bike log - handles both indoor and outdoor
    private var bikeLog = BikeLog()

    // Computed property to get the current bike log
    var currentBikeLog: BikeLog {
        return bikeLog
    }
...
// - RouteAnnotation
    ...
    override init() {
        ...
        if WCSession.isSupported() {
            ...
            print("📲 WCSession configured in BikeTrackingEngine")
        }
        ...
        // Add observer for bike settings changes
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handleBikeSettingsChange),
            name: .bikeSettingsDidChange,
            object: nil
        )
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            self.metricsCoordinator = MetricsCoordinator(bikeEngine: self)
        }
    ...
    @objc private func handleBikeSettingsChange() {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            
            // Reload settings from BikeSettingsManager or similar
            self.loadSettings()
    ...
    private func updateHeartRateMetrics(_ newHeartRate: Double) {
        ...
        func updateRunState(_ newState: BikeState) {
            let oldState = bikeState
            bikeState = newState
            ...
            if oldState == .active && (newState == .paused || newState == .notStarted || newState == .finished) {
                stopUpdatingMetrics()
            }
        }
    ...
    private func notifyObservers() {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(
                name: Notification.Name("BikeStateDidChange"),
                object: self
            )
    ...
    private func startUpdatingMetrics() {
        ...
        // Start location updates if this is an outdoor bike ride
        if !isIndoorMode {
    ...
    @objc private func handleLocationError(_ notification: Notification) {
        DispatchQueue.main.async { [weak self] in
            ...
            guard self.bikeState == .active else { return }
    ...
    @objc private func handleLocationPermissionDenied(_ notification: Notification) {
        DispatchQueue.main.async { [weak self] in
            ...
            if !self.isIndoorMode && self.bikeState == .active {
                // This is an outdoor bike ride with location denied - might need user attention
                print("⚠️ Location permission denied during outdoor bike ride - critical issue")
            }
        }
    }
    ...
    @objc private func applicationDidBecomeActive() {
        ...
        if bikeState == .active || bikeState == .paused {
    ...
    private func setupWatchConnectivity() {
        ...
        if WCSession.isSupported() {
            ...
            session.activate()
            print("📱 WCSession activated in BikeTrackingEngine")
        }
    }
    ...
    func session(_ session: WCSession, didReceiveMessage message: [String : Any], replyHandler: @escaping ([String : Any]) -> Void) {
        ...
        // Don't intercept requestActiveBikeWorkout - let it pass through to RunningWorkoutManager
        if messageType == "requestActiveBikeWorkout" {
            print("📱 BikeTrackingEngine: Handling requestActiveBikeWorkout directly")
            handleActiveWorkoutRequest(replyHandler: replyHandler)
            return
        }
        
        // Handle generic active workout request for bike
        if messageType == "requestActiveWorkout",
           (message["workoutType"] as? String)?.lowercased() == "bike" {
            print("📱 BikeTrackingEngine: Handling requestActiveWorkout for bike")
            let response = createActiveWorkoutResponse()
            replyHandler(response)
            return
        }
        
        // Handle syncWorkoutData specifically for already-joined workouts
        if messageType == "syncWorkoutData" && (bikeState == .active || bikeState == .paused) {
    ...
    public func processWatchMessage(_ message: [String: Any]) {
        ...
        print("📱 Current run state: \(bikeState.rawValue)")
        ...
        if let type = message["type"] as? String {
            ...
            if type == "syncWorkoutData" && (bikeState == .active || bikeState == .paused) {
                ...
                if let command = message["command"] as? String {
                    ...
                    case "paused":
                        if bikeState == .active {
                            ...
                            print("📱 🎯 Ignoring pause command - not currently running (state: \(bikeState.rawValue))")
                        }
                    case "inProgress":
                        if bikeState == .paused {
                            ...
                            print("📱 🎯 Ignoring resume command - not currently paused (state: \(bikeState.rawValue))")
                        }
                    case "completed":
                        if bikeState == .active || bikeState == .paused {
                            ...
                            print("📱 🎯 Ignoring end command - not currently active (state: \(bikeState.rawValue))")
                        }
                    default:
        ...
        if let requestDatabaseSave = message["requestDatabaseSave"] as? Int, requestDatabaseSave == 1 {
            print("📱 Watch is requesting database save - saving bike to AWS")
            
            // Save the bike to AWS
            saveBike()
...
//        handleTreadmillUpdate(message, replyHandler: { _ in })
    ...
    public func handleActiveWorkoutRequest(replyHandler: @escaping ([String: Any]) -> Void) {
        ...
        if bikeState == .active || bikeState == .paused {
            ...
            let workoutData: [String: Any] = [
                ...
                "workoutType": "bike",
                "state": bikeState == .active ? "inProgress" : (bikeState == .paused ? "paused" : "notStarted"),
                "bikeState": bikeState.rawValue,
    ...
    private func createActiveWorkoutResponse() -> [String: Any] {
        ...
        let hasActiveWorkout = (bikeState == .active || bikeState == .paused)
        
        print("📱 Run state: \(bikeState.rawValue), hasActiveWorkout: \(hasActiveWorkout)")
        ...
        let runTypeComponent = bikeType.rawValue
        ...
        let workoutData: [String: Any] = [
            ...
            "id": workoutId,
            "bikeState": bikeState.rawValue,
            "state": bikeState == .active ? "inProgress" : bikeState.rawValue,
            "workoutType": "bike",
    ...
    private func processWatchSyncData(_ message: [String: Any]) {
        ...
        if isWatchTracking && watchTime > 0 && (isIndoorMode || bikeState == .paused) {
    ...
    public func updateApplicationContext() {
        ...
        var contextData: [String: Any] = [
            ...
            "bikeType": bikeType.rawValue, // String
    ...
    func setup() {
        ...
        NotificationCenter.default.post(name: .didChangeRunState, object: nil, userInfo: ["state": bikeState.rawValue])
    ...
    public func startBike() {
        print("Starting bike ride of type: \(bikeType.rawValue)")
        
        // Initialize bikeId for AWS save
        print("📱 💾 INIT: Initializing bike activity for AWS save")
        bikeId = nil // Will be set after successful save
        
        // 🔧 FIX: Only reset metrics if NOT joining an existing workout
        if !isJoiningExistingWorkout {
            // Make sure metrics are reset to starting values for NEW bike rides
            ...
            startDate = Date()
            pauseDate = nil
            totalPausedTime = 0 // Reset accumulated pause duration for new bike ride
            
            print("📱 Started NEW bike ride - metrics reset to 0")
        } else {
        ...
        // Request location updates if needed for outdoor bike rides
        if !isIndoorMode {
            // Start location updates for outdoor bike ride
            locationManager.startUpdatingLocation()
        ...
        // Generate a new workout ID when starting a bike ride
        workoutId = UUID()
    }
    
    /// Start a new bike ride with specified bike type
    public func startBike(bikeType: BikeType) {
        // Set the bike type first
        self.bikeType = bikeType
        
        // Call the main startBike method
        startBike()
    }
    ...
    private func establishDeviceCoordination() {
        ...
        SmartHandoffCoordinator.shared.startMonitoring(workoutType: WorkoutType.biking)
    }
    
    /// Pause the current bike ride
    public func pauseBike() {
        // Only pause if running
        guard bikeState == .active else { return }
    ...
    /// Resume a paused bike ride
    public func resumeBike() {
        // Only resume if paused
        guard bikeState == .paused else { return }
    ...
    /// End the current bike ride
    public func endBike() {
        // Only end if running or paused
        guard bikeState == .active || bikeState == .paused else { return }
        ...
        print("📱 Bike ride ended - Settings restored")
    }
    
    // MARK: - Compatibility Wrappers (deprecated - use bike-specific methods)
    @available(*, deprecated, message: "Use pauseBike() instead")
    public func pauseRun() { pauseBike() }
    
    @available(*, deprecated, message: "Use resumeBike() instead")
    public func resumeRun() { resumeBike() }
    
    @available(*, deprecated, message: "Use endBike() instead")
    public func endRun() { endBike() }
    
    @available(*, deprecated, message: "Use endBike() instead")
    public func stopBike() { endBike() }
...
//        generateRunAnalysis()
        ...
        timer = Timer(timeInterval: 1.0, repeats: true) { [weak self] _ in
            ...
            if let startDate = self.startDate {
                ...
                if Int(rawElapsed) % 10 == 0 {
                    print("📱 ⏱️ TIMER DEBUG: rawElapsed=\(rawElapsed), pauseDuration=\(pauseDuration), totalPausedTime=\(self.totalPausedTime), bikeState=\(self.bikeState.rawValue)")
                }
                ...
                DispatchQueue.main.async {
                    ...
                    if Int(self.elapsedTime) % 3 == 0 {
                        ...
                        if self.bikeState == .active || self.bikeState == .paused {
    ...
    private func updateRunMetrics() {
        ...
        if isIndoorMode {
            collectBikeDataPoint()
        }
    }
    
    /// Collect a bike data point with current metrics
    private func collectBikeDataPoint() {
        // Only collect data points every 15 seconds to avoid too much data
        let currentTime = Date()
        if let lastDataPoint = bikeDataPoints?.last,
           currentTime.timeIntervalSince(lastDataPoint.timestamp) < 15.0 {
            return
        }
        
        // Initialize bike data points array if needed
        if bikeDataPoints == nil {
            bikeDataPoints = []
        }
        ...
        let dataPoint = BikeDataPoint(
            ...
            speed: speedKmh,
            pace: pace.value,
            power: nil, // Power not tracked currently
            resistance: nil // Resistance not tracked currently
        )
        
        bikeDataPoints?.append(dataPoint)
        
        // Keep only last 100 data points to prevent memory issues
        if let count = bikeDataPoints?.count, count > 100 {
            bikeDataPoints?.removeFirst()
        }
        
        print("📊 Collected bike data point: distance=\(distance.value)m, pace=\(pace.value)s/km, HR=\(heartRate)bpm, cadence=\(cadence)spm")
    }
    ...
    private func saveBike() {
        // Check if this is an indoor bike
        if isIndoorMode {
            // Save as indoor bike
            saveIndoorBike()
        } else {
            // Save as outdoor bike
            saveOutdoorBike()
        }
    }

    private func saveIndoorBike() {
        // Update cached average pace with this bike ride's data
        updateCachedPaceWithCurrentBike()
        
        // Format values
        let formattedTime = formattedTime
        let formattedPace = formattedPace
        ...
        // Format calories as rounded number
        let roundedCalories = round(calories)
        
        // Calculate average speed in m/s (convert from pace)
        var avgSpeed: Double? = nil
        if pace.value > 0 && elapsedTime > 0 {
            // Convert pace (seconds per km) to speed (m/s)
            avgSpeed = 1000.0 / pace.value
        }
        
        // Calculate max speed from cadence and pace (if available)
        let maxSpeed: Double? = avgSpeed // For indoor, use average as max
        
        // Ensure we have bike data points - create them if not available
        var bikeDataPoints: [BikeDataPoint] = []
        
        // Add starting point
        if let startDate = startDate {
            let startPoint = BikeDataPoint(
                timestamp: startDate,
                distance: 0.0,
        ...
        if elapsedTime > 30 {
            ...
            bikeDataPoints.append(midPoint)
        }
        
        // Add current/final point
        let currentPoint = BikeDataPoint(
            timestamp: Date(),
            distance: distance.value,
            heartRate: heartRate,
            cadence: cadence,
        ...
        bikeDataPoints.append(currentPoint)
        
        // Prepare activity data with indoor bike specifics
        ...
        let bikeDataPointsArray = bikeDataPoints.map { $0.toDictionary() }
        activityData["bikeDataPoints"] = bikeDataPointsArray
        print("📊 Saving \(bikeDataPointsArray.count) bike data points")
        
        // Populate bikeLog for display (unified structure)
        bikeLog.duration = formattedTime
        bikeLog.distance = distValue
        bikeLog.avgPace = formattedPace
        bikeLog.caloriesBurned = roundedCalories
        bikeLog.type = "indoor"
        bikeLog.bikeType = bikeType.rawValue
        bikeLog.bikeDataPoints = bikeDataPoints
        bikeLog.avgSpeed = avgSpeed
        bikeLog.maxSpeed = maxSpeed
        bikeLog.avgCadence = cadence > 0 ? cadence : nil
        ...
        bikeLog.createdBy = userId // Set Cognito user ID
        
        // Convert bikeType enum to string
        let bikeTypeString = isIndoorMode ? "indoor" : bikeType.rawValue.lowercased().replacingOccurrences(of: " ", with: "_")
        
        // Save to AWS
        ActivityService.shared.saveBike(
            userId: userId,
            duration: elapsedTime,
            ...
            elevationGain: nil, // Indoor bikes have no elevation
            elevationLoss: nil,
            routePoints: [], // Indoor bikes have no GPS route
            activityData: activityData,
            startLocation: nil,
            endLocation: nil,
            bikeType: bikeTypeString,
            avgSpeed: avgSpeed,
            maxSpeed: maxSpeed,
            avgPower: nil, // Not tracked currently
            avgCadence: cadence > 0 ? cadence : nil,
            isPublic: true,
            caption: nil
        ) { [weak self] result in
            guard let self = self else { return }
            ...
            case .failure(let error):
                print("📱 💾 INDOOR SAVE: ❌ Error saving indoor bike to AWS: \(error.localizedDescription)")
        ...
        print("📱 💾 INDOOR SAVE: ✅ Indoor bike save request sent to AWS")
    }

    private func saveOutdoorBike() {
        // Update cached average pace with this bike ride's data
        updateCachedPaceWithCurrentBike()
        ...
        // Calculate average and max speed from location data
        var avgSpeed: Double? = nil
        var maxSpeed: Double? = nil
        if !locationList.isEmpty {
            let speeds = locationList.compactMap { $0.speed > 0 ? $0.speed : nil }
            if !speeds.isEmpty {
        ...
        // If no speed from location, calculate from pace
        if avgSpeed == nil && pace.value > 0 {
            avgSpeed = 1000.0 / pace.value // Convert pace (seconds per km) to speed (m/s)
            maxSpeed = avgSpeed
        }
        
        // Prepare location data array for display
        ...
        bikeLog.distance = distValue
        bikeLog.avgPace = formattedPace
        bikeLog.type = "outdoor"
        bikeLog.bikeType = bikeType.rawValue
        bikeLog.locationData = locationDataArray
        bikeLog.caloriesBurned = roundedCalories
        bikeLog.elevationGain = elevationGainString
        bikeLog.elevationLoss = elevationLossString
        bikeLog.coordinateArray = locationList.map { ["latitude": $0.latitude, "longitude": $0.longitude] }
        bikeLog.avgSpeed = avgSpeed
        bikeLog.maxSpeed = maxSpeed
        bikeLog.avgCadence = cadence > 0 ? cadence : nil
        ...
        // Convert bikeType enum to string for API
        let bikeTypeString = bikeType.rawValue.lowercased().replacingOccurrences(of: " ", with: "_")
        ...
        ActivityService.shared.saveBike(
            ...
            switch result {
            case .success(let response):
                print("📱 💾 OUTDOOR SAVE: ✅ Bike saved to AWS successfully")
                print("📊 Saved with Distance: \(distValue)")
                print("📊 Saved with calories: \(roundedCalories)")
                ...
                if let activityId = response.data?.activityId {
                    ...
                    UserDefaults.standard.set(activityId, forKey: "lastSavedBikeId")
            ...
            case .failure(let error):
                print("📱 💾 OUTDOOR SAVE: ❌ Error saving bike to AWS: \(error.localizedDescription)")
        ...
        print("📱 💾 OUTDOOR SAVE: ✅ Bike save request sent to AWS")
        print("📊 Saved with \(locationList.count) location data points")
    }
    ...
    private func provideCoachingTip() {
        let tips = bikeType.coachingTips
    ...
    public func sendGroupRunInvitations(type: GroupRunType, scheduledStart: Date? = nil, distance: Double? = nil, message: String? = nil) {
        guard let currentUserId = UserIDHelper.shared.getCurrentUserID(),
              let currentUsername = UserIDHelper.shared.getCurrentUsername() else { return }
        
        // Create user model for current user (using Cognito user ID)
        var host = UserModel()
        host.userID = currentUserId
        host.userName = currentUsername
        host.name = CurrentUserService.shared.user.name ?? UserDefaults.standard.string(forKey: "name")
        host.profilePicture = CurrentUserService.shared.user.profilePicture ?? (UIImage(named: "default_avatar") ?? UIImage(systemName: "person.circle.fill")!)
        
        // Create the invitation
        let invitation = GroupRunInvitation(
            fromUser: host,
            runType: type,
            scheduledStart: scheduledStart,
            distance: distance != nil ? distance! / 1000 : nil, // Convert meters to km for display
            message: message,
        ...
        // Make sure we have a group run ID
        if groupRunId == nil {
            groupRunId = invitation.id.uuidString
        }
        
        // TODO: Implement AWS-based group run invitation system
        // For now, just log the invitation locally
        print("📱 Group bike ride invitation created:")
        print("   Type: \(type.rawValue)")
        print("   Group ID: \(groupRunId)")
        print("   Participants: \(groupRunParticipants.count)")
        
        // Send push notifications to participants (if implemented)
        self.sendInvitationPushNotifications(invitation: invitation)
    }
    ...
    private func sendParticipantStatusUpdate(_ status: ParticipantStatus) {
        guard let currentUser = UserIDHelper.shared.getCurrentUserID(),
        ...
        print("📱 Group bike ride status update:")
    ...
    func updateBikeType(_ newType: BikeType) {
        // Only update if the type has changed
        guard bikeType != newType else { return }
    ...
    private func sendTrackingStatusUpdate() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let currentRunState = self.bikeState
            let currentRunType = self.bikeType
            ...
            let message: [String: Any] = [
                ...
                "bikeState": currentRunState.rawValue,
                "metrics": metrics,
                "workoutActive": currentRunState == .active || currentRunState == .paused,
                "hasActiveWorkout": currentRunState == .active || currentRunState == .paused,
    ...
    private var bikeTracker: BikeTrackingEngine? {
        return self
    }
    ...
    func updateFormattedValues() {
        ...
        // Update bikeLog with current metrics
        updateBikeLogWithCurrentMetrics()
    }
    ...
    // Save incremental bike data to UserDefaults (lightweight state preservation)
    private func saveBikeData() {
        // Save basic metrics to UserDefaults for state preservation
        // This is a lightweight way to preserve state if app crashes
        print("📱 Saving incremental bike data to UserDefaults...")
        
        guard let userId = UserIDHelper.shared.getCurrentUserID() else {
            print("⚠️ No current user found for saving bike data")
            return
        }
        
        // Save to UserDefaults as a dictionary
        let bikeData: [String: Any] = [
            "userId": userId,
            "distance": distance.value,
            "duration": elapsedTime,
            "calories": calories,
        ...
        UserDefaults.standard.set(bikeData, forKey: "currentBikeData")
        UserDefaults.standard.synchronize()
        
        print("✅ Bike data saved to UserDefaults")
    }
    ...
    // Note: generateRunLog() removed - use generateBikeRideLog() instead
    
    func generateBikeRideLog() -> BikeRideLog? {
        // Make sure we have data to return
        guard !locationList.isEmpty else {
            print("Cannot generate BikeRideLog: no location data available")
            return nil
        }
        
        // Create a new BikeLog with current data (unified structure)
        var bikeLog = BikeLog()
        bikeLog.id = workoutId.uuidString
        bikeLog.duration = formatTime(Int(elapsedTime))
        bikeLog.distance = formatDistance(distance.value)
        bikeLog.avgPace = formatPace(pace.value)
        bikeLog.createdAt = startDate ?? Date()
        bikeLog.createdAtFormatted = formatDate(startDate ?? Date())
        bikeLog.createdBy = UserIDHelper.shared.getCurrentUserID()
        bikeLog.caloriesBurned = calories
        bikeLog.bikeType = bikeType.rawValue
        bikeLog.type = isIndoorMode ? "indoor" : "outdoor"
        
        // Add location data if available (outdoor only)
        if !isIndoorMode && !locationList.isEmpty {
            bikeLog.coordinateArray = locationList.map { location in
                ["latitude": location.latitude, "longitude": location.longitude]
            }
            bikeLog.locationData = locationList.map { location in
                [
                    "latitude": location.latitude,
                    "longitude": location.longitude,
                    "altitude": location.altitude,
    ...
    private func updateBikeLogWithCurrentMetrics() {
        ...
        if !locationList.isEmpty {
            ...
            // Convert to coordinateArray format (array of [latitude, longitude] dictionaries)
            bikeLog.coordinateArray = locationList.map {
    ...
    func sendWorkoutUpdate(forced: Bool = false) {
        ...
        if bikeState == .finished {
            let finalWorkoutData: [String: Any] = [
                "type": "workoutUpdate",
                "id": workoutId.uuidString,
                "workoutType": "bike",
                "bikeType": "outdoorBike",
                "state": "completed",
                "bikeState": "completed",
                "elapsedTime": elapsedTime,
                "isIndoor": false,
        ...
        if timeSinceLastUpdate < 1.0 && !forced && !isForcedUpdate && bikeState != .finished {
        ...
        let hasValidMetrics = elapsedTime > 0 || distance.value > 0 || bikeState == .active || isAutoJoin
        
        if !hasValidMetrics && bikeState == .notStarted && !isAutoJoin {
        ...
        var workoutData: [String: Any] = [
            ...
            "workoutType": "bike",
            "bikeType": "outdoorBike",
            "state": bikeState.rawValue,
            "bikeState": bikeState.rawValue,
            "elapsedTime": elapsedTime,
            "isIndoor": false,
            "timestamp": Date().timeIntervalSince1970,
            "workoutActive": bikeState == .active || bikeState == .paused,
            "hasActiveWorkout": bikeState != .notStarted,
            "isDashboardMode": isDashboardMode,
            "isWatchTracking": isWatchTracking,
            "hasGoodLocationData": hasGoodLocationData,
            "useImperialUnits": !useMetric
        ...
        // Use self.isPrimaryFor* properties instead of WorkoutCommunicationHandler
        if !isPrimaryForDistance {
        ...
        // Add authority flags
        workoutData["isPrimaryForDistance"] = !isPrimaryForDistance
        workoutData["isPrimaryForPace"] = !isPrimaryForPace
        workoutData["isPrimaryForHeartRate"] = !isPrimaryForHeartRate
        workoutData["isPrimaryForCalories"] = !isPrimaryForCalories
        workoutData["isPrimaryForCadence"] = !isPrimaryForCadence
        ...
        print("📱 Sending workout update to watch: state=\(bikeState.rawValue), time=\(elapsedTime)s, distance=\(distance.value)m")
    ...
    // MARK: - Bike Saving Methods
    
    /// Incrementally saves the current bike state without finalizing the bike workout.
    /// Used for checkpointing bike state during app transitions or potential interruptions.
    func saveIncremental() {
        // Only save if there's an active bike workout
        guard bikeState == .active || bikeState == .paused else { return }
        
        print("📱 Saving incremental bike state...")
        
        // Save bike data to AWS
            self.saveBikeData()
        
        // Cache the latest data to UserDefaults as a fallback
    ...
    private func saveStateToUserDefaults() {
        ...
        // Save basic bike state data
        defaults.set(workoutId.uuidString, forKey: "BikeTracker.workoutId")
        // Note: BikeState doesn't have rawValue, so we'll use a string representation
        defaults.set(String(describing: bikeState), forKey: "BikeTracker.bikeState")
        defaults.set(bikeType.rawValue, forKey: "BikeTracker.bikeType")
        defaults.set(elapsedTime, forKey: "BikeTracker.elapsedTime")
        defaults.set(distance.value, forKey: "BikeTracker.distance")
        defaults.set(pace.value, forKey: "BikeTracker.pace")
        defaults.set(calories, forKey: "BikeTracker.calories")
        defaults.set(Date().timeIntervalSince1970, forKey: "BikeTracker.lastSaved")
    ...
    func restoreIncrementalState() -> Bool {
        ...
        guard let lastSavedTime = defaults.object(forKey: "BikeTracker.lastSaved") as? TimeInterval else {
            print("No saved bike state found")
            return false
        }
        ...
        guard let savedWorkoutIdString = defaults.string(forKey: "BikeTracker.workoutId"),
        ...
        // Restore run state
//        if let rawBikeState = defaults.string(forKey: "BikeTracker.bikeState"),
//           let restoredRunState = bikeState(rawValue: rawBikeState) {
//            bikeState = restoredRunState
//        }
//        
//        // Restore run type
//        if let rawRunType = defaults.string(forKey: "BikeTracker.bikeType"),
//           let restoredRunType = BikeType(rawValue: rawRunType) {
//            bikeType = restoredRunType
//        }
        
        // Restore metrics
        elapsedTime = defaults.double(forKey: "BikeTracker.elapsedTime")
        distance = Measurement(value: defaults.double(forKey: "BikeTracker.distance"), unit: UnitLength.meters)
        pace = Measurement(value: defaults.double(forKey: "BikeTracker.pace"), unit: UnitSpeed.minutesPerKilometer)
        calories = defaults.double(forKey: "BikeTracker.calories")
        ...
        print("Restored bike in progress: \(formattedDistance) \(distanceUnit) in \(formattedTime)")
        
        // Notify observers that we've restored state
        NotificationCenter.default.post(name: .bikeTrackingEngineDidUpdateState, object: self)
    ...
    private func clearSavedState() {
        let defaults = UserDefaults.standard
        defaults.removeObject(forKey: "BikeTracker.workoutId")
        defaults.removeObject(forKey: "BikeTracker.bikeState")
        defaults.removeObject(forKey: "BikeTracker.bikeType")
        defaults.removeObject(forKey: "BikeTracker.elapsedTime")
        defaults.removeObject(forKey: "BikeTracker.distance")
        defaults.removeObject(forKey: "BikeTracker.pace")
        defaults.removeObject(forKey: "BikeTracker.calories")
        defaults.removeObject(forKey: "BikeTracker.lastSaved")
        defaults.synchronize()
    }
    ...
    public func handleWatchCommunicationError(_ error: Error) {
        ...
        if watchCommErrorCount >= maxWatchCommErrors && !watchCircuitBreakerOpen {
            ...
            DispatchQueue.main.async {
                ...
                // Update the delegate
                self.delegate?.bikeTrackingEngineDidUpdateState(self)
   ...
   public func importWorkoutFromWatch(
    ...
    // Immediately update bikeLog with imported data
       updateBikeLogWithCurrentMetrics()
       ...
       let locationHistory = rawData["locationHistory"] as? [[String: Any]] {
        ...
        for locationPoint in locationHistory {
            ...
            locationDataArray.append(locationData)
              ...
              let bikeDataPointsArray = rawData["bikeDataPoints"] as? [[String: Any]] {
        
        // Process bike data points for indoor bikes
        var dataPoints: [BikeDataPoint] = []
        
        for point in bikeDataPointsArray {
            ...
            // Bike-specific optional fields
            let power = point["power"] as? Double
            let resistance = point["resistance"] as? Double
        ...
        // Store bike data points for later use when saving to AWS
        self.bikeDataPoints = dataPoints
    }
    ...
    private func handleRunHistoryRequest(limit: Int, replyHandler: @escaping ([String: Any]) -> Void) {
        ...
        if ![].isEmpty {
            ...
            sendCombinedRunHistoryToWatch(outdoorRuns: [], limit: limit, replyHandler: replyHandler)
            return
        }
        
       
        // Fetch bike history from AWS
        print("📱 ❌ NO CACHED BIKE LOGS FOUND")
        print("📱 🌐 FETCHING FRESH BIKE HISTORY FROM AWS...")
        
        guard let userId = UserIDHelper.shared.getCurrentUserID() else {
            print("📱 ❌ No user ID available")
            replyHandler(["status": "error", "message": "No user ID available"])
            return
        }
        
        // Fetch bikes from AWS using ActivityService
        ActivityService.shared.getBikes(
            ...
            guard let self = self else {
                print("📱 ❌ BikeTrackingEngine deallocated during database fetch")
                replyHandler(["status": "error", "message": "BikeTrackingEngine deallocated"])
                return
            }
            ...
            case .success(let response):
                guard let data = response.data else {
                    ...
                    replyHandler(["status": "error", "message": "No data in response"])
                    return
                }
                
                // Convert AWS activities to BikeRideLog
                var bikeLogs: [BikeRideLog] = []
                for activity in data.activities {
                    if let bikeLog = self.convertAWSActivityToBikeLog(activity) {
                        bikeLogs.append(bikeLog)
                    }
                }
                
                print("📱 ✅ SUCCESSFULLY FETCHED \(bikeLogs.count) BIKE LOGS")
                
                // Send results to watch
                self.sendCombinedRunHistoryToWatch(outdoorRuns: bikeLogs, limit: limit, replyHandler: replyHandler)
                
            case .failure(let error):
                print("📱 ❌ ERROR FETCHING BIKE LOGS: \(error.localizedDescription)")
                replyHandler(["status": "error", "message": error.localizedDescription])
            }
        }
    }
    
    /// Convert AWS activity to BikeRideLog
    private func convertAWSActivityToBikeLog(_ activity: AWSActivity) -> BikeRideLog? {
        ...
        if let elevationLoss = activity.elevationLoss {
            bikeLog.elevationLoss = String(format: "%.0f", elevationLoss * 3.28084) // Convert meters to feet
        }
        ...
        if let activityDataString = activity.activityData,
           ...
           let type = json["type"] as? String {
            ...
            // Default to outdoor if we have location data, indoor otherwise
            bikeLog.type = (bikeLog.locationData?.isEmpty == false) ? "outdoor" : "indoor"
        }
    ...
    private func sendCombinedRunHistoryToWatch(outdoorRuns: [BikeRideLog], limit: Int, replyHandler: @escaping ([String: Any]) -> Void) {
        ...
        for runLog in outdoorRuns {
            ...
            if let runType = runLog.bikeType {
                logData["bikeType"] = runType
            } else {
                logData["bikeType"] = "outdoorBike"
            }
            
            //TODO: add heart rate to BikeRideLog
...
//            }
    ...
    private func updateCachedPaceWithCurrentBike() {
        // Calculate pace for current bike ride
        ...
        print("📱 Updated cached average pace with current bike ride: \(paceSecondsPerKm) s/km")
    }
    ...
    DispatchQueue.main.async { [weak self] in
        ...
        if workoutState == "completed" {
            ...
            if forcedStateChange && (self.bikeState == .active || self.bikeState == .paused) {
                ...
                let bikeType: BikeType = bikeType
                ...
                // End the bike workout, which will save to AWS
                self.endBike()
                
                // Send success response with database save confirmation
                replyHandler([
                    "status": "success",
                    "message": "Bike workout ended and saved",
        ...
        // Send response
        replyHandler(["status": "success", "phoneState": self.bikeState.rawValue])
    }
}
    ...
    public func handleActiveRunningWorkoutRequest(replyHandler: @escaping ([String: Any]) -> Void) {
        ...
        let immediateAck: [String: Any] = [
            "status": "received",
            "timestamp": Date().timeIntervalSince1970,
            "hasActiveWorkout": (bikeState == .active || bikeState == .paused),
            "useImperialUnits": !useMetric
        ]
    ...
    public func ensureTimerIsRunning() -> Bool {
        if timer == nil && (bikeState == .active || bikeState == .paused) {
    ...
    private func calculatePauseDuration() -> TimeInterval {
        ...
        if bikeState == .paused, let pauseDate = self.pauseDate {
...
// MARK: - BikeState Extension for Raw Value

extension BikeState: RawRepresentable {
...
extension BikeTrackingEngine {
    ...
    func initiateHandoffToWatch(completion: @escaping (Bool) -> Void) {
        guard bikeState != .notStarted else {
            completion(false)
            return
        }
        ...
        let handoffMessage = WorkoutHandoffMessage(
            ...
            workoutId: workoutId.uuidString,
            metrics: metrics,
            state: convertBikeStateToWorkoutState(bikeState),
        ...
        if session.isReachable {
            session.sendMessage(handoffMessage.toDictionary(), replyHandler: { response in
                ...
                print("❌ [BikeTrackingEngine] Handoff failed: \(error.localizedDescription)")
                completion(false)
            })
        } else {
            do {
    ...
    func handleHandoffFromWatch(_ message: [String: Any]) {
        ...
        if bikeState != .notStarted {
            handleHandoffConflict(phoneState: convertBikeStateToWorkoutState(bikeState),
        ...
        bikeState = convertWorkoutStateToBikeState(handoffMessage.state)
    ...
    private func handleHandoffConflict(phoneState: WorkoutState, watchWorkout: WorkoutHandoffMessage) {
        ...
        bikeState = convertWorkoutStateToBikeState(watchWorkout.state)
    }
    ...
    private func updateMetricsFromExternalDevices(_ metrics: WorkoutMetrics) {
        ...
        let isIndoor = bikeType == .indoorBike
        
        if let source = coordinationEngine.selectBestDataSource(
            for: MetricType.heartRate,
            workoutType: WorkoutType.biking,
            isIndoor: isIndoor,
            availableSources: [MetricsSource.watch, MetricsSource.oura, MetricsSource.garmin, MetricsSource.fitbit]
        ), source == MetricsSource.watch || source == MetricsSource.oura || source == MetricsSource.garmin || source == MetricsSource.fitbit {
        ...
        if let source = coordinationEngine.selectBestDataSource(
            for: MetricType.cadence,
            workoutType: WorkoutType.biking,
            isIndoor: isIndoor,
            availableSources: [MetricsSource.watch, MetricsSource.garmin]
        ), source == MetricsSource.watch || source == MetricsSource.garmin {
    ...
    private func convertBikeStateToWorkoutState(_ bikeState: BikeState) -> WorkoutState {
        switch bikeState {
    ...
    private func convertWorkoutStateToBikeState(_ workoutState: WorkoutState) -> BikeState {
        switch workoutState {
        case .idle: return .notStarted
        case .starting, .running: return .active
        case .paused: return .paused
        case .stopping, .stopped, .completed: return .finished
        }
    }
    ...
    func getHandoffData() -> [String: Any]? {
        guard bikeState != .notStarted else { return nil }
